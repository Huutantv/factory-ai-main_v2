//! What happens between launching `fauto` and the first prompt: the identity banner, first-run
...
/// + load them, or dismiss (won't nag again — `fauto mcp trust` re-enables). MCP servers can run
...
pub(crate) fn prompt_mcp_trust(server_count: usize) {
    ...
    if ok {
        ...
        println!(
            "{}",
            style("✓ trusted — its tools are now available.").color256(theme::ACCENT)
        );
    } else {
        let _ = crate::agent::mcp::dismiss_project();
        println!(
            "{}",
            style("skipped — run `fauto mcp trust` anytime to enable.")
                .color256(crate::ui::theme::FAINT)
        );
    }
}
...
/// up front so it shows exactly once (even if the user Ctrl-C's mid-setup); `fauto config` reruns setup.
pub(crate) async fn first_run_onboarding() {
    ...
    if !proceed {
        println!(
            "\n{}",
            style("No problem — run `fauto config` whenever you're ready. Type /help inside for a tour.")
                .color256(crate::ui::theme::FAINT)
        );
        return;
    }
    ...
    if let Err(e) = config_ui::config_wizard().await {
        ...
        eprintln!(
            "{}",
            style("You can finish later with `fauto config`.").color256(crate::ui::theme::FAINT)
        );
        return;
    }
    ...
    println!(
        "\n{} {}",
        style("✓ You're all set.").color256(theme::ACCENT).bold(),
    ...
    println!(
        "{}",
        style("Tip: add specialist sub-agents with `fauto agents install msitarzewski/agency-agents`.")
            .color256(crate::ui::theme::FAINT)
    );
}
...
pub(crate) fn identity_banner() -> (String, Vec<String>) {
    ...
    if let Some(l) = crate::features::zones::quick_legacy_probe() {
        notes.push(format!(
            "⚠ legacy zone {l} has data — `fauto zone migrate` merges it into {slug}"
        ));
    }
    (main, notes)
}
...
///
/// Called from `main` rather than the REPL banner: `fauto memory list` is a CLI path that never

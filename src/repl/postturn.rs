...
/// FREE — regex extraction, no model call — through the SAME pipeline as `fauto memory learn`
...
/// (tokens_before, tokens_after). Same core the agent loop uses, so the REPL and `fauto serve` compact
/// identically.

...
//! the specialist prompt), so `fauto agents install msitarzewski/agency-agents` makes all of them
...
//! HOME, and fauto-native beats claude-compat.
//!
...
pub enum AgentSource {
    ...
    ClaudeHome,
    /// `~/.aizen/agents` — the personal (HOME) dir; `fauto agents install` writes here.
    AizenHome,
    /// `<repo>/.claude/agents` — ecosystem read-compat, project-local.
    ClaudeProject,
    /// `<repo>/.aizen/agents` — fauto-native, project-local. Highest precedence.
    AizenProject,
}
...
impl AgentSource {
    pub fn label(self) -> &'static str {
        match self {
            ...
            AgentSource::AizenHome => "fauto-home",
            AgentSource::ClaudeProject => "claude-project",
            AgentSource::AizenProject => "fauto-project",
        }
    }
}
...
/// higher-precedence dir WINS on a slug collision (project beats HOME; fauto beats claude). Never errors.
...
/// Per-source-dir file counts (for `fauto agents where`). Raw counts (pre-dedup), so the same agent
...
/// Render a full card for `fauto agents show`.
...
/// `fauto agents set-model` are the model-only surface and shouldn't have to spell out three
...
fn write_enabled(set: &HashSet<String>) -> Result<()> {
    ...
    let mut body = String::from(
        "# Specialist agents pinned to the always-on <agents> index (one slug per line).\n\
         # Edit freely, or use `fauto agents enable/disable`. Unlisted agents are still\n\
...
mod tests {
    ...
    fn with_sandbox<T>(tag: &str, f: impl FnOnce(&Path) -> T) -> T {
        ...
        let root = std::env::temp_dir().join(format!("fauto-agents-{tag}-{}", std::process::id()));
    ...
    fn aizen_wins_over_claude() {
        with_sandbox("aizenwins", |root| {
            ...
            write_file(
                &root.join(".aizen/agents"),
                "dup.md",
                &card("Dup", "fauto version", "a"),
            );
            assert_eq!(
                load("dup").unwrap().description,
                "fauto version",
                "fauto-native beats claude-compat"
            );
        });
    }
    ...
    fn full_four_dir_precedence_list_and_load_agree() {
        with_sandbox("fourdir", |root| {
            ...
            assert_eq!(
                from_list.description, "fauto-proj",
                "highest precedence wins in list"
            );
            assert_eq!(
                from_load.description, "fauto-proj",

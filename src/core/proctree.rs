//! Process-tree containment and tree-kill for every child F.Auto spawns through a shell.
//!
...
/// console window of its own. Applied to every child F.Auto spawns through [`prepare`] /
/// [`prepare_tokio`]. Two reasons, both load-bearing:
///
///  * No console flash when F.Auto runs under a TUI/alt-screen or fully headless (`fauto serve`).
...
mod tests {
    ...
    /// The shell wrapper F.Auto actually uses, so the test exercises the real spawn shape.

//! Persistent CLI endpoint config (`fauto config`) — base URL / API key / default model stored at
...
pub struct CliConfig {
    ...
    /// The fauto analogue of Claude Code's `ultracode` (xhigh + standing orchestration permission).
    ...
    /// Telegram bot integration (the `fauto serve` daemon + telegram_send/telegram_ask tools).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Skill marketplace base URL for `skill_search`/`skill_install` (and `fauto skill search/install`).
    ...
    /// it ever touches a descriptive `phase: …` milestone, so `fauto time list` stays readable.
    /// `None` ⇒ default 5.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Discord two-way bot (the `fauto discord serve` gateway daemon). Distinct from the one-way Discord
    ...
    /// TUI layout name (opencode-style): built-in `default` / `balanced` / `polished` / `dense` / `spacious`,
    /// `auto`, or a custom layout under `~/.aizen/layout/*.{json,jsonc}`. `None` ⇒ `balanced`.
...
///
/// Why this exists: `cli-config.json` is a single file shared by every fauto window on the machine,
...
/// non-REPL caller (one-shot `fauto -p`, cron jobs, the hostbot daemon, sub-agents), which have no
            ...
            if !WARNED.swap(true, Ordering::Relaxed) {
                ...
                crate::ui::tui::note_line(&format!(
                    "warning: {} is corrupt ({e}); using defaults. A .bak copy is kept before any \
                     re-save — run `fauto config` to repair.",
                    path.display()
                ));
            }
            CliConfig::default()
...
mod tests {
    ...
    fn self_review_resolution_honors_explicit_values_and_oracle_default() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-self-review-{}", std::process::id()));
    ...
    fn role_provider_uses_profile_and_model_override() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-roles-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn model_endpoint_registry_routes_endpoint_with_model() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-mep-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn subagent_endpoint_folds_role_default_through_registry() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-subep-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
        let main = ResolvedEndpoint {
            base_url: "https://parent/v1".into(),
    ...
    fn round_trips_through_disk() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-cfg-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn auto_copy_defaults_on_and_honours_field_and_env() {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-auto-copy-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn save_normalizes_legacy_approval_fields() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-approval-cfg-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn branded_env_reads_aizen_only() {
        ...
        // The pre-rebrand `NG_*` name is NOT read anymore (fully rebranded to F.Auto).

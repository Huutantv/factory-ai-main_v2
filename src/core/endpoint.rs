...
pub(crate) fn resolve_endpoint(
    ...
    let base_url = base_url
        ...
        .or(cfg.base_url)
        .context("no base URL — run `fauto config` (interactive setup), or pass --base-url / set AIZEN_BASE_URL")?;
    let api_key = api_key
        .or_else(|| cli_config::branded_env("API_KEY"))
        .or(cfg.api_key)
        .context("no API key — run `fauto config` (interactive setup), or pass --api-key / set AIZEN_API_KEY")?;
    ...
    let model = model
        ...
        .or(cfg.model)
        .context("no model — run `fauto config` (interactive setup) or `fauto models` to list, or pass --model / set AIZEN_MODEL")?;
...
pub(crate) fn resolve_base_key(
    ...
    let base_url = base_url
        .or_else(|| cli_config::branded_env("BASE_URL"))
        .or(cfg.base_url)
        .context("no base URL — run `fauto config`")?;
    let api_key = api_key
        .or_else(|| cli_config::branded_env("API_KEY"))
        .or(cfg.api_key)
        .or_else(|| codex_oauth_api_key(&base_url))
        .context("no API key — run `fauto config`")?;
    Ok((base_url, api_key))
}

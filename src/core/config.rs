...
/// in one, else the current dir. Repo-root-aware so launching `fauto` from a SUBDIR still finds the
...
/// accented path gets a readable zone instead of a shredded one — `~/Dự án/fauto` keyed
/// `d-n-fauto-…` before.
///
...
/// and [`legacy_slug_candidates_for`] emits the pre-fold spelling so `fauto zone migrate` can still
...
/// checkout is now an explicit `fauto zone migrate`, never an implicit re-key. Cached per
...
/// slug itself is excluded; `fauto zone migrate` merges artifacts found under the rest.
...
/// Mid-confidence learned candidates land here for `fauto memory review` (P3).
...
/// so other F.Auto tooling can reuse a downloaded model. Consumed by the `dense` feature build.
...
/// The env var stays on top so a one-off `AIZEN_EMBED_MODEL=… fauto …` still wins over the file, and
...
mod tests {
    ...
    fn workspace_key_never_carries_the_verbatim_spelling() {
        let dir = std::env::temp_dir().join(format!("fauto-wskey-{}", std::process::id()));
    ...
    fn legacy_candidates_cover_url_root_and_subdir_launch_eras() {
        let base = std::env::temp_dir().join(format!("fauto-legacy-{}", std::process::id()));
    ...
    fn project_slug_is_stable_and_follows_ng_project_root() {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-slug-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        ...
        let dir2 = std::env::temp_dir().join(format!("fauto-slug2-{}", std::process::id()));

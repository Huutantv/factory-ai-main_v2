...
//!
//! F.Auto needs to know which physical machine it runs on so that `tier: device`
...
//! ## History (`device.json`)
//!
//! F.Auto stores **every** device id it has ever seen (not just the current one) so
        ...
        Err(_) => {
            ...
            match probe_fallback() {
                ...
                Err(_) => {
                    ...
                    (
                        RawSecret::Text("fauto-device-fallback".to_string()),
                        "fallback",
                    )
                }
            }

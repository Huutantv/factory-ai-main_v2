//! `fauto bench memory` — anti-oracle recall bench.
//!
...
/// Entry point for `fauto bench memory`.
pub fn run(split: &str, update_baseline: bool, hybrid: bool, fuzzy: bool) -> Result<()> {
    ...
    if split == "gate" || split == "all" {
        ...
        match std::fs::read_to_string(baseline_path()) {
            ...
            Err(_) => {
                println!(
                    "no baseline yet — run `fauto bench memory --update-baseline` to capture one."
                );
            }
        }
    }
...
/// `fauto bench memory --evolution`. A 6-session reuse simulation: signal facts (genuinely-useful

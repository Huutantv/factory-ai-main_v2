...
//!   Area). Crisp, monochrome, single-cell — they inherit the palette accent instead of a
//!   clashing multi-colour emoji, so the whole TUI reads as one coherent palette (the "clean like a
...
pub fn cost() -> &'static str {
    pick("💰", "\u{f155}") // nf-fa-dollar — the per-turn tokens line
}

/// The tool-call row's ONE glyph, grouped by what the tool touches (files / shell / web / search /
/// memory). The transcript's busiest line gets exactly one glyph — unknown tools keep the plain
/// `⚙` (text, not PUA: it renders on any font, exactly as before this grouping existed). The `off`
/// tier is handled by the caller (`tool_icon`).
...
mod tests {
    ...
    #[test]
    fn tool_kind_groups_known_tools_and_keeps_the_gear() {
        // default (nerd) tier: PUA glyphs, one cell each (the transcript row must stay aligned);
        // unknown tools keep the plain text gear that rendered before grouping existed.
        assert_eq!(tool_kind("file_read"), "\u{f07b}");
        assert_eq!(tool_kind("shell_run"), "\u{f120}");
        assert_eq!(tool_kind("web_search"), "\u{f0ac}");
        assert_eq!(tool_kind("browser_navigate"), "\u{f0ac}");
        assert_eq!(tool_kind("memory_search"), "\u{f1c0}");
        assert_eq!(tool_kind("search_files"), "\u{f002}");
        assert_eq!(tool_kind("todo_write"), "⚙");
        for g in ["\u{f07b}", "\u{f120}", "\u{f002}", "\u{f0ac}"] {
            let c = g.chars().next().unwrap();
            assert!(('\u{e000}'..='\u{f8ff}').contains(&c), "{c:?} not PUA");
        }
    }
}

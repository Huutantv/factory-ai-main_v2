...
//! near = bright).
//!
//! `fauto art` renders one frame to stdout; the same [`frame`] builder is what the idle screensaver
//! can drive later by advancing `phase`.
//!
//! On sixel-capable terminals (Windows Terminal, WezTerm, iTerm2, foot, …) `fauto art` instead
...
/// `fauto art` — show the moonscape. A true raster image on sixel-capable terminals (Windows

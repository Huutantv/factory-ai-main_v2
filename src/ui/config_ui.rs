//! The interactive **configuration surface**: `fauto config` / `fauto auth` and every editor the
//! `/config` menu opens.
//!
...
pub(crate) async fn run_auth(cmd: AuthCmd) -> Result<()> {
    match cmd {
        AuthCmd::Login { provider } => {
            ...
            println!("  next: fauto config  → pick 'ChatGPT Codex (experimental)' or:");
            println!(
                "        fauto config set --base-url {} --api-key codex-oauth --model {}",
...
pub(crate) async fn run_config(cmd: Option<ConfigCmd>) -> Result<()> {
    let cmd = match cmd {
        Some(c) => c,
        None => return config_wizard().await, // bare `fauto config` → interactive setup
    };
    match cmd {
        ConfigCmd::Set {
            base_url,
            ...
            if !any_scalar_flag && !any_role_flag && model_endpoint.is_empty() {
                anyhow::bail!(
                    "nothing to set — pass at least one flag; `fauto config set --help` lists them"
                );
            }
            ...
            if any_role_flag {
                ...
                for (role, provider, ..) in &role_flags {
                    ...
                    if !known.iter().any(|k| k == name) {
                        anyhow::bail!(
                            "no saved provider profile '{name}' for role {role} — see `fauto config provider list`"
                        );
                    }
                }
            ...
            println!(
                "{} {}",
                crate::ui::theme::ok("✓"),
                style("saved").color256(theme::ACCENT)
            );
            print_config(&cfg);
            Ok(())
        }
    ...
    match cmd {
        ...
        ProviderConfigCmd::List => {
            let Some(list) = cfg.providers.as_ref() else {
                println!("no provider profiles — add one with `fauto config provider add`");
                return Ok(());
            };
            for p in list {
...
/// Render the saved config as a grouped, aligned "Studio" panel: an accent title rule with the file
/// path, then sections (Endpoint / Session / Cost / Display) of `key   value` rows where the value's
/// colour carries meaning (accent = a chosen value, green = on/ok, faint = off/unset). Shown at the end
/// of the wizard, after `config set`, and on `fauto config show`.
///
...
fn print_config(cfg: &cli_config::CliConfig) {
    ...
    // ── header: "config" on the left, the file path faint on the right, then an accent rule ──
    let title = "config";
        ...
        row(
            ...
            if crate::llm::oauth_codex::has_token() {
                ...
                format!(
                    "{}  {}",
                    theme::warn("!"),
                    theme::faint(format!("{detail} · fauto auth login codex"))
                )
            },
        );
    }
...
/// `fauto config` / `/config` / menu → Setup. A fresh install (no endpoint yet) gets the guided
...
/// A yes/no toggle with the shared accent theme (used by the section editors below).
...
///
/// Anthropic is here as an OpenAI-COMPATIBLE entry, which is worth being precise about: fauto speaks
...
// so the one-shot `fauto config` path is unchanged.
...
async fn prompt_validated_api_key(
    ...
    // `fauto auth login codex`. Prompting here would ask for a secret that does not exist and then
    ...
    if crate::llm::oauth_codex::is_codex_base_url(base) {
        if crate::llm::oauth_codex::has_token() {
            ...
            line_warn("no Codex login found yet — run `fauto auth login codex` before using this provider");
        }
...
/// "Done" exits. Every field here is also scriptable via `fauto config set`, so nothing depends on it.
...
async fn prompt_codex_login(theme: &ColorfulTheme) -> Result<Option<&'static str>> {
    ...
    match oauth_codex::login_interactive().await {
        ...
        Err(e) => {
            ...
            if yn(
                theme,
                "Keep this provider anyway (finish later with `fauto auth login codex`)?",
                true,
            )? {
                Ok(Some(PLACEHOLDER))
            } else {
...
async fn config_edit_agent_pins(cfg: &mut cli_config::CliConfig) -> Result<()> {
    ...
    if all.is_empty() {
        line_warn("no specialists installed — `fauto agents install msitarzewski/agency-agents`");
        return Ok(());
    }
    if cfg.providers.as_ref().is_none_or(Vec::is_empty) {
        line_warn("no saved providers — add one first");
...
async fn config_setup_full(cfg: &mut cli_config::CliConfig) -> Result<()> {
    ...
    tui::emit_line(&format!(
        "{}",
        style("F.Auto · setup").bold().color256(theme::ACCENT)
    ));
    tui::emit_line(&format!(
        "{}",
    ...
    // Group the steps under accent section headers so the flow reads as Connection → Model → Behavior.
    let step = |label: &str| {
        tui::emit_line(&format!(
            "\n{} {}",
            style("◆").color256(theme::ACCENT),
            style(label).color256(theme::ACCENT).bold()
        ));
    };
    ...
    if cfg.model.is_none() {
        anyhow::bail!("a model is required (run `fauto models` to list them)");
    }
    ...
    tui::emit_line(&format!(
        "\n{} {}",
        crate::ui::theme::ok("✓"),
        style("Saved.").color256(theme::ACCENT).bold()
    ));
    print_config(cfg);
    tui::emit_line(&format!(
        "{}",
        style("Ready — type a message, or run:  fauto chat -p \"hello\"")
            .color256(crate::ui::theme::FAINT)
    ));
    Ok(())
}

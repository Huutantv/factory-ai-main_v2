...
pub(crate) fn read_input_box(history: &[String]) -> Result<Option<(String, Vec<String>)>> {
    ...
    // Live palette colours: dim for the box frame, the full accent for the `❯` prompt — the same
    // split the retained input box uses.
    let accent = crate::ui::theme::accent_dim_idx();
    let prompt_accent = crate::ui::theme::accent_idx();
    let bar = |l: &str, r: &str| {
    ...
    let render = |chars: &[char], cursor: usize, scroll: &mut usize| -> (String, usize) {
        ...
        let line = format!(
            ...
            arrow = style("❯").color256(prompt_accent).bold(),
            sp = " ".repeat(pad)
        );

...
async fn discord_test() -> Result<()> {
    let (client, _) =
        discord::configured().context("Discord bot not set up — run `fauto discord setup`")?;
    let name = client.get_me().await?;
    println!(
        "{}",
        style(format!("✓ bot token valid — @{name}")).color256(theme::ACCENT)
    );
    Ok(())
}
...
fn discord_status() {
    ...
    println!("{}", style("Discord bot").bold().color256(theme::ACCENT));
    println!("token:    {token}");
...
fn discord_disable() -> Result<()> {
    ...
    println!(
        "{}",
        style("Discord bot disabled (config removed).").color256(theme::ACCENT)
    );
    Ok(())
}
...
async fn discord_setup() -> Result<()> {
    ...
    println!(
        "\n{}",
        style("Discord bot setup").bold().color256(theme::ACCENT)
    );
    println!(
        "{}",
    ...
    println!(
        "{}",
        style(format!("✓ bot @{name}")).color256(theme::ACCENT)
    );
    ...
    println!(
        "\n{}",
        style("Saved. Start the bot with:  fauto discord serve").color256(theme::ACCENT)
    );
    Ok(())
}
...
async fn telegram_test() -> Result<()> {
    ...
    client
        .send_message(chat, "✅ F.Auto test message — Telegram is wired up.")
        .await?;
    println!(
        "{}",
        style(format!("sent a test message to chat {chat}")).color256(theme::ACCENT)
    );
    Ok(())
}
...
fn telegram_disable() -> Result<()> {
    ...
    println!(
        "{}",
        style("Telegram disabled (bot config removed).").color256(theme::ACCENT)
    );
    Ok(())
}

/// An F.Auto "connected app" surfaced in the `/apps` hub. Telegram is two-way (a long-poll daemon +
...
impl Integration {
    ...
    fn blurb(&self) -> &'static str {
        match self {
            ...
            Integration::Telegram => "control fauto from your phone (bot + approval prompts)",
...
async fn app_catalog_menu() -> Result<()> {
    ...
    // Rows: featured apps first, then any connected custom apps (added via `fauto apps add <name>`).
    struct Row {
        key: String,
        label: String,
        icon: String,
...
/// `/apps → Discord` — Discord can be a two-way BOT (receives + replies, needs `fauto discord serve`)
...
async fn discord_app_menu() -> Result<()> {
    ...
    let items = [
        ...
        "Test the bot token",
        "Start the bot daemon  (fauto discord serve — Ctrl-C to stop)",
...
/// `/apps` — the integrations hub: F.Auto's connected apps (Telegram today; Discord/Slack/webhooks
...
async fn webhook_app_setup(ch: notify::Channel) -> Result<()> {
    ...
    if notify::is_configured(ch) {
        ...
        .await
        {
            Ok(()) => println!(
                "{}",
                style(format!("✓ test delivered to {}", ch.label())).color256(theme::ACCENT)
            ),
...
async fn webhook_app_test(ch: notify::Channel) -> Result<()> {
    ...
    match notify::send_to(ch, "🔔 F.Auto test notification.").await {
        Ok(()) => println!(
            "{}",
            style(format!("✓ delivered to {}", ch.label())).color256(theme::ACCENT)
        ),
...
fn webhook_app_disable(ch: notify::Channel) -> Result<()> {
    ...
    println!(
        "{}",
        style(format!("{} disabled (URL removed).", ch.label())).color256(theme::ACCENT)
    );
    Ok(())
}
...
fn skill_new_interactive() -> Result<()> {
    ...
    println!(
        "{}",
        style(format!("saved skill → {}", path.display())).color256(theme::ACCENT)
    );
    Ok(())
}
...
fn skill_restore_interactive(retired: &[skill::Skill]) {
    ...
    match skill::restore(&names[i]) {
        Ok(_) => println!(
            "{}",
            style(format!("restored '{}'", names[i])).color256(theme::ACCENT)
        ),
        Err(e) => tui::note_line(&format!("{} {e}", style("skill:").red())),
    }
}
...
async fn skill_search_interactive() -> Result<()> {
    ...
    println!(
        "{} '{}'.",
        style("✓ installed").color256(theme::ACCENT),
        sk.name
    );
    Ok(())
}
...
fn persona_new_interactive() -> Result<()> {
    ...
    println!(
        "{}",
        style(format!("saved persona → {}", path.display())).color256(theme::ACCENT)
    );
    Ok(())
}
...
async fn persona_paste_interactive(history: &mut Vec<Message>, model: &str) -> Result<()> {
    ...
    println!(
        "{}",
        style(format!("created persona '{name}' → {}", path.display())).color256(theme::ACCENT)
    );
    println!(
        "  {} {}",
        style("role:").dim(),
        ...
        .interact()?
    {
        ...
        println!(
            "{}",
            style(format!("now playing: {name}")).color256(theme::ACCENT)
        );
    }
    Ok(())
}
...
pub(crate) fn persona_self_view_n(slug: &str, name: &str, all: bool) {
    ...
    if ins_n >= persona::self_mem::INSIGHT_CAP {
        ...
        println!(
            "\n{}",
            style(format!(
                "insights are at the {} cap — `fauto persona forget <id>` retires one (archived)",
    ...
    if !arch.is_empty() {
        println!(
            "{}",
            style(format!(
                "{} retired — `fauto persona unforget <id>` brings one back",
                arch.len()
            ))
            .dim()
        );
...
pub(crate) async fn personas_menu(history: &mut Vec<Message>, model: &str) -> Result<()> {
    loop {
        ...
        if pick < n {
            ...
            println!(
                "{}",
                style(format!("now playing: {}", personas[pick].name)).color256(theme::ACCENT)
            );
            return Ok(());
        }
        match actions[pick - n].as_str() {
            ...
            "Use default voice (no persona)" => {
                ...
                println!(
                    "{}",
                    style("persona cleared → default assistant voice").color256(theme::ACCENT)
                );
                return Ok(());
            }
            "Delete a persona" => {
                    ...
                    match persona::delete(&names[i]) {
                        Ok(true) => {
                            ...
                            println!(
                                "{}",
                                style(format!(
                                    "retired '{}' — `fauto persona restore {}` brings it back",
                                    names[i], names[i]
                                ))
                                .color256(theme::ACCENT)
                            );
...
/// `/telegram` — a dedicated sub-menu for the Telegram integration (one of F.Auto's connected
...
pub(crate) async fn telegram_menu() -> Result<()> {
    ...
    let items = [
        ...
        "Send a test message",
        "Status",
        "Start daemon  (control fauto from your phone — Ctrl-C to stop)",
...
async fn telegram_setup() -> Result<()> {
    ...
    println!(
        "\n{}",
        style("Telegram setup").bold().color256(theme::ACCENT)
    );
    println!(
        "{}",
    ...
    println!(
        "{}",
        style(format!("✓ bot @{username}")).color256(theme::ACCENT)
    );
    ...
    println!(
        "{}",
        style(format!("✓ captured chat id {chat}")).color256(theme::ACCENT)
    );
    ...
    let _ = client
        .send_message(
            chat,
            "✅ F.Auto connected. Run `fauto serve`, then send /help.",
        )
        .await;
    println!(
        "\n{}",
        style("Saved. Start the daemon with:  fauto serve").color256(theme::ACCENT)
    );
    Ok(())
}
...
async fn poll_for_chat_id(client: &telegram::Client) -> Result<i64> {
    ...
    anyhow::bail!("timed out waiting for a message — run `fauto telegram setup` again")
}

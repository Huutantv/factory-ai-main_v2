...
/// 50%, WARN yellow from 50%, ERR red from 80% — the same green/yellow/red meanings the rest of
...
pub(crate) fn print_cost(history: &[Message], model: &str) {
    ...
    if calls > 0 {
        ...
        let mut line = format!(
            ...
            style("💰 session usage").color256(theme::ACCENT).bold(),
            fmt_k(p as usize),
            fmt_k(c as usize),
        ...
        match (cfg.price_in, cfg.price_out) {
            (Some(pin), Some(pout)) => {
                ...
                line.push_str(&format!(
                    "  ·  {}",
                    style(format!("est ${cost:.4} (@ ${pin}/${pout} per 1M in/out)")).color256(theme::ACCENT)
                ));
            }
        ...
        tui::emit_line(&format!(
            ...
            style("📊 estimated").color256(theme::ACCENT).bold(),
            fmt_k(est),
            fmt_k(window),
...
pub(crate) fn print_context(history: &[Message], model: &str) {
    ...
    tui::emit_line(&format!(
        "{}  {model} · window {}{}",
        style("📊 context breakdown").color256(theme::ACCENT).bold(),
        fmt_k(window),
        if auto { "" } else { " (est)" },
    ));
    ...
    tui::emit_line(&format!(
        ...
        style(format!("{:<26}", "total"))
            ...
            .bold(),
        style(format!("~{} / {} tok", fmt_k(total), fmt_k(window))).color256(theme::ACCENT),
    ));
}

...
//!     runtime, which would break the headless static binary `fauto serve` ships as — so it's
...
mod tests {
    ...
    fn temp_png(tag: &str) -> std::path::PathBuf {
        ...
        let p = std::env::temp_dir().join(format!("fauto-img-{tag}-{}.png", std::process::id()));
    ...
    fn image_file_to_data_url_reads_png() {
        ...
        let txt = std::env::temp_dir().join(format!("fauto-not-img-{}.png", std::process::id()));

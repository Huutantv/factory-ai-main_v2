//! The F.Auto landing splash — a hermes-style title + a bordered panel listing the agent's tools
//! and the CLI's commands. The wordmark rides the palette's title gradient and each tool group takes
    ...
    if allow_sixel && logo_is_sixel() {
        ...
        for line in sun_lines() {
            let _ = writeln!(
                out,
                " {}",
                style(&line).color256(crate::ui::theme::accent_idx()).bold()
            );
        }
    }
}

/// Append the Factory Auto logo: the sun mark, then the block-art wordmark (a theme gradient), then the
/// tagline.
fn push_title(out: &mut String, word: &str, allow_sixel: bool) {
    out.push('\n');
    push_sun(out, allow_sixel);
    out.push('\n');
    let ramp = crate::ui::theme::title_ramp();
    for (row, ramp_color) in ramp.iter().enumerate() {
        let line: String = word
            .chars()
            .map(|c| glyph(c)[row])
            .collect::<Vec<_>>()
            .join(" ");
        let _ = writeln!(out, "  {}", style(line).color256(*ramp_color).bold());
    }
    let _ = writeln!(
        out,
        "  {}",
        style("FACTORY AUTO").color256(crate::ui::theme::tone_idx(1))
    );
    out.push('\n');
}
...
fn render_inner(allow_sixel: bool) -> String {
    ...
    for (gi, (label, items)) in TOOL_GROUPS.iter().enumerate() {
        let tone = crate::ui::theme::tone_idx(gi);
        boxline(
            &mut out,
            &format!(
                "  {}{} {}",
                crate::ui::icons::g(crate::ui::icons::tool_group(label)),
                style(format!("{label:<9}")).color256(tone).bold(),
                style(*items).color256(crate::ui::theme::accent_dim_idx())
            ),
        );
    }
    #[cfg(feature = "browser")]
    for line in wrap_items(BROWSER_TOOLS, ", ", INNER.saturating_sub(14)) {
        boxline(
            ...
            &format!(
                ...
                style(format!("{:<9}", "browser"))
                    .color256(crate::ui::theme::tone_idx(TOOL_GROUPS.len()))
                    .bold(),
                style(line).color256(crate::ui::theme::accent_dim_idx())
            ),
        );
    }
    boxblank(&mut out);
    ...
    for line in wrap_items(COMMANDS, " · ", INNER - 2) {
        boxline(
            &mut out,
            &format!("  {}", style(line).color256(crate::ui::theme::tone_idx(3))),
        );
    }
    boxblank(&mut out);
...
pub fn welcome() -> String {
    ...
    let _ = writeln!(
        out,
        "  {}",
        style("Welcome to F.Auto — your agentic coding companion.")
            .color256(crate::ui::theme::accent_idx())
            .bold()
    );
    let _ = writeln!(
...
mod tests {
    ...
    /// Count distinct 256-colour SGR foregrounds in a rendered splash. The splash must paint
    /// several distinct hues — that is the whole point of the multi-hue panel.
    fn distinct_fg_colours(s: &str) -> usize {
        ...
        while i + 4 < bytes.len() {
            if bytes[i] == 0x1b && bytes[i + 1] == b'[' {
                ...
                if let Some(end) = rest.find('m') {
                    let seq = &rest[..end];
                    // The 256-colour foreground may be combined with modifiers (`1;38;5;213`), so
                    // locate the `38;5;` marker rather than requiring it at the start.
                    if let Some(pos) = seq.find("38;5;") {
                        let digits: String = seq[pos + 5..]
                            .chars()
    ...
    #[test]
    fn splash_is_multi_hue() {
        // The test harness has no TTY, so `console` strips colour by default. Force it on for this
        // render (and restore after), or we would be counting zero hues.
        ...
        assert!(
            out.contains("38;5;213"),
            "the accent/title ramp must appear in the splash"
        );
        let hues = distinct_fg_colours(&out);
        assert!(
            hues >= 5,
            "the splash should paint several distinct hues, got {hues}"
        );
    }

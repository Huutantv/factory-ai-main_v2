...
pub(crate) fn resolve_turn_effort(line: &str) -> Option<String> {
    // Ultimate mode pins max effort every turn (auto-detect is bypassed) — the fauto `ultracode`.
...
/// accent · low = green · medium = dim · high = yellow) so the whole effort feature reads as
...
pub(crate) fn effort_turn_line(eff: Option<&str>) -> String {
    // low = green, medium = dim; the three "hot" rungs escalate high → xhigh → max
    // (yellow → bold yellow → red) so the eye can tell them apart at a glance.
...
fn apply_effort_choice(idx: usize) {
    ...
    match cli_config::save(&cfg) {
        Ok(_) => tui::emit_line(&style(msg).color256(theme::ACCENT).to_string()),
        Err(e) => tui::emit_line(&format!("{} {e}", style("effort:").red())),
    }
    // Recolour the retained input box, exactly as `/ultimate` does: warn-yellow while ultimate is
    // ON, accent-dim when OFF. Reads the effective state, so an `AIZEN_ULTIMATE` in the environment
    // keeps the box warn-yellow even after the slider was dragged down from the top.
...
mod tests {
    ...
    fn isolated(tag: &str) -> std::sync::MutexGuard<'static, ()> {
        ...
        let home = std::env::temp_dir().join(format!("fauto-effort-{tag}-{}", std::process::id()));

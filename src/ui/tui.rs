...
//! When the TUI isn't active (the one-shot `fauto chat`/`agent` subcommands, pipes, CI) every entry
...
/// wiped. Outside the REPL (one-shot `fauto agent`, pipes, CI) it degrades to `eprintln!`, keeping
...
/// when the TUI owns the screen, else `eprintln!` to stderr so a one-shot `fauto agent` keeps stdout
...
pub fn tool_call_end(
    ...
    if retained::is_running() {
        ...
        for line in
            retained::render_tool_row(&ev, width(), crate::ui::layout::current().compact_tools)
                .split('\n')
        {
...
/// Recolour the retained input box for ultimate mode (warn-yellow ON, accent-dim OFF). No-op on the classic
...
/// Lines moved per scroll step — one shared step for the mouse wheel, PageUp and PageDown, so
/// every scroll gesture moves the same distance.
...
fn handle_retained_mouse(
    ...
    match kind {
        ...
        MouseEventKind::ScrollUp if !*dragging_scrollbar => {
            retained::scroll(-(SCROLL_STEP as i32)); // negative delta = up, toward history
        }
        MouseEventKind::ScrollDown if !*dragging_scrollbar => {
            retained::scroll(SCROLL_STEP as i32); // positive delta = down, toward the live tail
        }
    ...
    loop {
        ...
        if retained::is_active() && RETAINED_INFO_OVERLAY.load(Ordering::Relaxed) {
            match key {
                ...
                Key::PageUp => retained::scroll(-(SCROLL_STEP as i32)),
                Key::PageDown => retained::scroll(SCROLL_STEP as i32),
        ...
        match key {
            ...
            Key::Char('\u{10}') => {
                // Ctrl-P: readline-style previous-history recall. Mirrors ↑ but skips draft-line
                // walking (a control chord is never caret motion). Open @-picker / slash-palette
                // keeps the key, like the arrows yield to it.
                let at_len = { at_matches(&render().lock().unwrap().draft).len() };
                let pal = {
                    let r = render().lock().unwrap();
                    slash_matches(&r.draft).len()
                };
                if at_len == 0 && pal == 0 && !history.is_empty() {
                    recall_history_prev(&mut hist_idx, &mut draft_saved, &history);
                }
            }
            Key::Char('\u{0e}') => {
                // Ctrl-N: readline-style next-history recall. Mirrors ↓ the same way.
                let at_len = { at_matches(&render().lock().unwrap().draft).len() };
                let pal = {
                    let r = render().lock().unwrap();
                    slash_matches(&r.draft).len()
            ...
            Key::PageUp if retained::is_active() => {
                retained::scroll(-(SCROLL_STEP as i32));
            }
            Key::PageDown if retained::is_active() => {
                retained::scroll(SCROLL_STEP as i32);
            }
            Key::End if retained::is_active() => {
                // Context-aware End: with text before the caret it means "to end-of-draft" (a
                // second press then reaches the tail); with an empty draft — or a caret already
                // at the end — it means "back to the live tail". Documented in REFERENCE.md.
                let at_end = {
                    let r = render().lock().unwrap();
                    r.draft.is_empty() || r.cursor >= r.draft.len()
                };
                if at_end {
                    retained::scroll_end();
                } else {
...
// auto is calm accent, low goes green (light & cheap), medium dim, high burns the reserved
// warn-yellow (runs hot). Runs while the sticky box is SUSPENDED (or in the plain REPL), so it owns stdin;
...
const E_DESCS: [&str; 7] = [
    ...
    "max effort + auto-launches workflows — fauto's ultracode",
];
...
/// The palette colour for a tier: auto = accent, low = green (ok), medium = dim,
/// high/xhigh = the reserved warm yellow (matches the `⚡ yolo` "runs hot" cue), max/ultimate = red
/// (the hottest end). high/xhigh share the yellow and max/ultimate share the red; within each pair
...
mod tests {
    ...
    fn e_color_maps_each_tier_to_a_palette_role() {
        // auto→accent, low→ok(green), medium→dim, high/xhigh→warn(yellow), max/ultimate→err(red).
        ...
        assert_eq!(e_color(4), theme::WARN, "xhigh shares high's yellow");
        assert_eq!(e_color(5), theme::ERR);
        assert_eq!(e_color(6), theme::ERR, "ultimate shares max's red");
    }

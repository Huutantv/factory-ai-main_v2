...
//! - **Gutter.** Every assistant line carries a magenta `▌ ` gutter — a continuous bar that marks
...
/// A left-edge bar marking assistant output. Takes the palette's ASSISTANT role colour (magenta).
fn gutter() -> String {
    format!("{} ", theme::assistant("▌"))
}
...
impl MarkdownStream {
    ...
    /// `cols` floors at COMMON_FLOOR (8) so every surface shares one narrow-width policy.
    ...
    fn render_line(&mut self, line: &str, out: &mut String) {
        ...
        if ls.is_empty() {
            out.push_str(theme::assistant("▌").to_string().as_str());
            out.push('\n');
            return;
        }
    ...
    /// already carry the accent gutter bar so the left edge stays continuous. `style_each` styles the
    ...
    /// content. Capped so wide terminals don't stretch code across the whole screen, and derived from
    /// `cols` (not a fixed floor) so a narrow pane never forces the box wider than the viewport:
    /// gutter(2) + `│ `(2) + content + ` │`(2) always fits.
    ...
    fn fence_inner(&self) -> usize {
        self.cols.saturating_sub(8).clamp(1, 76)
    }
...
/// The narrowest a table column may be shrunk to before the box gives up — COMMON_FLOOR.
const TABLE_MIN_COL: usize = 8;

/// Per-column natural VISIBLE width: the widest cell (post-`inline`) in each column, header included.
fn natural_widths(table: &MarkdownTable) -> Vec<usize> {
...
/// Fit a table's columns into `budget` visible cells so it can render as a BOX instead of
/// immediately collapsing to the stacked list. `Some(widths)` means every column got at least
/// `TABLE_MIN_COL` cells (cells wider than their column wrap onto multiple lines — see
/// `render_table_tty`); `None` means the pane cannot hold one minimum column per cell and the
/// caller must fall back to `render_table_stacked_tty`. When the natural widths already fit they
/// are returned untouched, so a table that always fitted renders byte-identical to before.
///
/// Allocation is water-filling: every column starts at `min(natural, TABLE_MIN_COL)`, then the
/// leftover pool is handed out in ascending order of NEED (each column may take at most an equal
/// share of what remains at its turn). Narrow columns therefore keep their natural width — they
/// cost almost nothing — while the wide columns that caused the overflow share what is left
/// evenly, instead of being clipped by a flat proportional cut.
fn fit_column_widths(table: &MarkdownTable, budget: usize) -> Option<Vec<usize>> {
    let natural = natural_widths(table);
    let n = natural.len();
    // Frame overhead: one `─`-run of width+2 per column, joined by `┬`/`┼`/`┴`, plus the corners.
    let avail = budget.checked_sub(n * 3 + 1)?;
    if natural.iter().sum::<usize>() <= avail {
        return Some(natural);
    }
    let base: Vec<usize> = natural.iter().map(|w| (*w).min(TABLE_MIN_COL)).collect();
    let pool = avail.checked_sub(base.iter().sum::<usize>())?;
    let need: Vec<usize> = natural
        .iter()
        .enumerate()
        .map(|(i, w)| w.saturating_sub(base[i]))
        .collect();
    ...
    for (k, &i) in order.iter().enumerate() {
        // Equal share of what is left, among this column and every later (≥-need) one.
        let share = left / (order.len() - k);
        let give = need[i].min(share);
        widths[i] += give;
        left -= give;
    }
    Some(widths)
...
/// One cell's rendered lines, wrapped to `width` visible cells. The WRAP measures the raw marker
/// text (deliberate, and identical to the stacked renderer's contract): a wrapped line's post-
/// `inline()` width is always ≤ its raw width, so a raw-width wrap can only UNDER-fill a column,
/// never overrun it — and `display_pad` re-pads by the visible width, so borders stay aligned.
/// A cell that already fits visible-width renders as ONE styled line (markers stripped first),
/// which keeps every natural-width table byte-identical to the pre-wrap renderer.
fn cell_lines(cell: &str, width: usize) -> Vec<String> {
    if measure_text_width(&inline(cell)) <= width {
        return vec![inline(cell).to_string()];
    }
    let mut wrapped = wrap_plain(cell, width);
    if wrapped.is_empty() {
        wrapped.push(String::new());
    }
    wrapped
        .into_iter()
        .map(|l| inline(&l).to_string())
        .collect()
}

fn render_table_tty(table: &MarkdownTable, cols: usize) -> String {
    let budget = cols.saturating_sub(measure_text_width(&gutter())).max(8);
    // One policy for every column count and width: fit the columns (shrinking + wrapping cells),
    // and fall back to the stacked list ONLY when the pane cannot hold one minimum column per
    // cell. The old rule (`> 5 headers || natural width > budget`) dumped a six-column table to
    // `◆ 1` blocks on an 80-col terminal that had ample room for a shrunk, wrapped box.
    let Some(widths) = fit_column_widths(table, budget) else {
    ...
    // Header and body rows are multi-line: every column of a row wraps to its fitted width, and
    // the row is as tall as its tallest cell (short cells' columns pad with blanks).
    let header_cells: Vec<Vec<String>> = table
        .headers
        .iter()
        .enumerate()
        .map(|(i, cell)| cell_lines(cell, widths[i]))
        .collect();
    let header_h = header_cells.iter().map(|c| c.len()).max().unwrap_or(1);
    for ln in 0..header_h {
        out.push_str(&gutter());
        out.push_str(&theme::accent_dim("│").to_string());
        for (i, cell) in header_cells.iter().enumerate() {
            out.push(' ');
            out.push_str(
                &theme::accent(display_pad(
                    cell.get(ln).map(String::as_str).unwrap_or(""),
                    widths[i],
                    TableAlign::Center,
                ))
                .bold()
...
mod tests {
    ...
    #[test]
    fn narrow_width_matrix_never_overflows() {
        // Phase 11 width matrix: prose, code fence and table must all fit at 16 cols (the
        // COMMON_FLOOR is 8; 16 is the smallest realistic terminal).
        let cols = 16;
        let mut s = MarkdownStream::new(true, cols);
        let mut out = s.push("a fairly long prose line that must wrap\n");
        out.push_str(&s.push("```rs\nlet xxxxxxxxxx = 1;\n```\n"));
        out.push_str(&s.push("| A | B |\n|---|---|\n| x | y |\n"));
        out.push_str(&s.finish());
        let plain = strip_ansi_codes(&out).to_string();
        for l in plain.lines() {
            assert!(
                measure_text_width(l) <= cols,
                "row overflowed at 16 cols: {l:?}"
            );
        }
    }
    ...
    /// The screenshot bug: a 6-column table with long cells dumped to `◆ 1` stacked blocks on an
    /// 80-col terminal that had ample room for a shrunk, wrapped box. Fitting (shrink + wrap) must
    /// keep it a box at every realistic width, with no row wider than the pane.
    #[test]
    fn wide_content_tables_stay_a_box_at_realistic_widths() {
        let md = "| Thành phần | File | Vai trò | Trạng thái | Ghi chú | Tỷ lệ |\n\
                  |---|---|---|---|---|---:|\n\
                  | Điều phối chính | main.py | VisionController (QObject) nối UI với pipeline inspection | ổn | God object | 33% |\n\
                  | Camera | camera_backend.py | Basler pypylon và OpenCV USB (QThread worker, trigger, timeout) | ổn | dual backend | 21% |\n";
        for cols in [80, 120] {
            let mut s = MarkdownStream::new(true, cols);
            let mut out = s.push(md);
            out.push_str(&s.finish());
            let plain = strip_ansi_codes(&out).to_string();
            assert!(
                plain.contains('╭') && plain.contains('┼') && !plain.contains("◆ 1"),
                "must render a box at {cols} cols, got stacked:\n{plain}"
            );
            assert!(
                plain.contains("QThread") && plain.contains("pipeline") && plain.contains("God"),
                "cell content survives at {cols} cols:\n{plain}"
            );
            for l in plain.lines() {
                assert!(
                    measure_text_width(l) <= cols,
                    "row wider than {cols} cols: {l:?} = {}",
                    measure_text_width(l)
                );
            }
            // Every box row (borders and cells) shares one visible width — multi-line rows may not
            // drift the closing │.
            let box_w: Vec<usize> = plain
                .lines()
                .filter(|l| {
            ...
            assert!(
                ...
                "drifting borders at {cols}: {plain}"
            );
        }
        // 40/60 cols cannot hold even 9-wide minimum for 6 columns (6×11+1 > pane): the honest
        // fallback is the stacked list, and it must still carry every cell's content.
        for cols in [40, 60] {
            let mut s = MarkdownStream::new(true, cols);
            let mut out = s.push(md);
            out.push_str(&s.finish());
            let plain = strip_ansi_codes(&out).to_string();
            assert!(
                plain.contains("VisionController") && plain.contains("camera_backend.py"),
                "stacked fallback keeps content at {cols} cols:\n{plain}"
            );
            assert!(plain.lines().all(|l| measure_text_width(l) <= cols));
        }
    }

    #[test]
    fn too_many_columns_still_fall_back_to_stacked() {
        // 9 columns of minimum width cannot fit a 60-col pane: the stacked renderer remains the
        // honest answer — content is never silently clipped away.
        let cells = |v: &str| format!("|{}|", vec![v; 9].join("|"));
        let md = format!(
            "{}\n{}\n{}",
            cells(" AAAAAAAAAA "),
            cells("---"),
        ...
        let mut s = MarkdownStream::new(true, 60);
        let mut out = s.push(&md);
        out.push_str(&s.finish());
        let plain = strip_ansi_codes(&out).to_string();
        assert!(
            plain.contains("◆ 1") && plain.contains("AAAAAAAAAA"),
            "unfittable table must stack with content intact:\n{plain}"
        );
        assert!(plain.lines().all(|l| measure_text_width(l) <= 60));
    }

    #[test]
    fn wrapped_tables_are_chunk_invariant() {
        // Streaming a table that WRAPS must produce byte-identical output regardless of where the
        // chunk boundary falls (the live transcript streams; the replay must match it exactly).
        let md = "| A | B | C |\n|---|---|---|\n\
                  | một chuỗi rất dài để buộc cột phải wrap lại trong hộp | ngắn | xxxxxxxxxxxxxxxxxxxxxxxxx |\n";
        let mut s = MarkdownStream::new(true, 48);
        let mut expected = s.push(md);
        expected.push_str(&s.finish());
        for cut in 0..=md.len() {

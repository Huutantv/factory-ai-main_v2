//! Retained full-frame terminal backend — the only chat surface fauto has. (The classic sticky
...
struct CacheKey {
    ...
    hash: u64,
    complete: bool,
    /// Active palette tag — cached rows bake SGR colours at render time, so the key carries the
    /// palette identity even though the palette is compiled in (keeps the key honest if roles change).
    theme_tag: u64,
...
impl RenderCache {
    fn get_or_render(&mut self, block: &UiBlock, width: u16) -> Vec<String> {
        // Palette + compact flag participate so a `/layout` switch (which changes the tool-row
        // shape) re-renders instead of serving stale cached rows.
        ...
        let rows = match &block.payload {
            Payload::Text(s) => match block.kind {
                ...
                // Generic notices (endpoint fallbacks, sandbox advisories, token lines) arrive as
                // single long rows; wrap them to the pane width so they read as a paragraph instead
                // of being clipped mid-word at the frame edge.
                BlockKind::Generic => sanitize_keep_sgr(s)
                    .split('\n')
                    .flat_map(|row| wrap_sgr_row(row, w))
                    .collect(),
                // Intro is a pre-formatted box (splash panel): wrapping would destroy its borders,
                // so over-wide rows are clipped with an ellipsis instead.
                BlockKind::Intro => sanitize_keep_sgr(s)
                    .split('\n')
                    .map(|row| clip_to(row, w))
                    .collect(),
            ...
            Payload::Tool(t) => render_tool_row(t, w, crate::ui::layout::current().compact_tools)
                .split('\n')
                .map(str::to_string)
                .collect(),
...
struct AppState {
    ...
    /// Ultimate mode ON → the input box (prompt arrow + framing rules) recolours to the reserved
    /// warn-yellow,
...
enum Command {
    ...
    WorkCaption(String),
    /// Ultimate mode toggled — recolour the input box (warn-yellow ON, accent-dim OFF). Pushed from the
...
fn render_loop(rx: Receiver<Command>, ready: Sender<bool>, intro: String, status: String) {
    ...
    // Footer height at the last draw. A footer height CHANGE is the one event under which stale
    // ghosts survive ratatui's cell diff: an out-of-band writer (the post-draw hyperlink injector,
    // a terminal-side reflow, a child writing to the console) leaves the REAL screen ahead of
    // ratatui's buffer, and once rows shift by a footer row the diff's "unchanged" comparison
    // compares the wrong rows against each other — leaving two strings interleaved on one line
    // (`que6ed`-style ghosts). Clearing on exactly this transition keeps the repaint cost to the
    // rare working↔idle / grow-shrink edges instead of repainting every frame.
    let mut last_footer: Option<(u16, bool, bool, bool, bool)> = None;
    loop {
        ...
        if dirty {
            if let Some(s) = session.as_mut() {
                ...
                if let Some(idx) = state.screensaver {
                    ...
                    // Composer identity changed (working→idle placeholder↔draft swap, queued/steer
                    // counts, [Nimg] chip, composer grow/shrink): the rows the composer paints
                    // shift content — the edge under which an out-of-band desync (post-draw
                    // hyperlink injector, terminal-side reflow, a child printing) leaves two
                    // frames interleaved on one line (`que6ed`-style ghosts). Dropping the diff
                    // baseline here repaints every cell, same precedent as Ctrl-L above.
                    if let Some(area) = after {
                        let sig = (
                            footer_height(&state, area.width, area.height),
                            state.working,
                            state.input.draft.is_empty(),
                            state.input.images > 0,
                            state.input.queued_count > 0,
                        );
                        if last_footer != Some(sig) {
                            if last_footer.is_some() {
                                let _ = s.terminal.clear();
                            }
                            last_footer = Some(sig);
                        }
                    }
...
fn apply_command(state: &mut AppState, cmd: Command) {
    match cmd {
        Command::Emit(s) => {
            ...
            // and the green/red edit diff survive into styled spans — only cursor moves / erases

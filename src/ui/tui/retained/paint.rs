...
pub(super) fn draw(frame: &mut Frame<'_>, state: &mut AppState) {
    ...
    let layout = input_layout(
        state,
        area.width as usize,
        max_input_rows(area.height, layout_cfg.max_input_rows),
    );
...
/// How many text rows the composer may occupy in a terminal `height` rows tall, given the
/// active layout's `max_input_rows` cap.
///
...
/// The footer height [`draw`] will choose, computed OUTSIDE the frame so the render loop can
/// detect a footer height CHANGE — the one event under which stale-screen ghosts survive ratatui's
/// cell diff (an out-of-band writer that left the real screen ahead of the buffer makes
/// "unchanged" cells at shifted rows never get repainted; see the run loop's clear-on-change).
/// Pure over `AppState` exactly like `draw`'s own sizing call — the two cannot disagree.
pub(super) fn footer_height(state: &AppState, width: u16, height: u16) -> u16 {
    let layout_cfg = crate::ui::layout::current();
    let layout = input_layout(
        state,
        width as usize,
        max_input_rows(height, layout_cfg.max_input_rows),
    );
    footer_chrome_rows(&layout_cfg)
        .saturating_add(layout.rows.len() as u16)
        .min(height.saturating_sub(1).max(1))
}

pub(super) fn max_input_rows(height: u16, cap: usize) -> usize {
    ...
    // The active layout caps the composer height (`dense` = 5); the hard ceiling stays. `cap` is
    // passed in (rather than read) so this stays pure over its inputs.
    let cap = (cap as u16).clamp(1, MAX_INPUT_TEXT_ROWS);
...
pub(super) fn draw_transcript(frame: &mut Frame<'_>, area: Rect, state: &mut AppState) {
    ...
    // between steps. Blue caption (the fauto link-blue) so it reads as "live status", not transcript
...
fn styled_row(kind: BlockKind, row: String) -> Line<'static> {
    match kind {
        ...
        // assistant `▌` gutter keeps its own colour because it rode through as SGR; uncoloured text
...
/// brand-bloom spinner + a typewriter-revealed caption in the fauto link-blue, then the elapsed clock.
///
...
pub(super) fn working_line(state: &AppState) -> Vec<(String, Line<'static>)> {
    ...
    let elapsed = state
        ...
        .unwrap_or(0);
    // `spinner caption   12s · Esc to stop` — spinner in accent (the structural brand colour),
    // caption in the reasoning/live-status hue (purple, distinct from grey transcript prose), the
    // elapsed clock + stop hint faint at the tail so they inform without pulling the eye.
    //
    ...
    let spans = vec![
        ...
        Span::styled(revealed, Style::default().fg(Color::Indexed(pal.reasoning))),
        Span::styled(
...
pub(super) fn draw_footer(
    ...
    // Input-box accent: warn-yellow while ultimate mode is ON (tying the box to the `✦ ultimate` chip so the
    // heightened mode is unmistakable at a glance), the usual accent-dim otherwise. Drives the
    ...
    // `console::truncate_str` subtracts the ellipsis width from the limit UNGUARDED: a limit
    // below the ellipsis panics in debug (and the chip+meter alone claim the whole row on
    // ~40-column panes), so at those widths the HUD status drops and the chip keeps the row.
    let left = if left_budget < 2 {
        String::new()
    } else {
        console::truncate_str(&state.input.status, left_budget, "…").into_owned()
    };
    let gap = width.saturating_sub(console::measure_text_width(&left) + right_w);
    let mut hud_spans = vec![
        Span::styled(left, Style::default().fg(Color::Indexed(pal.header))),
        Span::raw(" ".repeat(gap)),
    ];
...
/// cell from the text. The tag is reserved first: it is always painted when rows are hidden (clipped
/// to the width on a very narrow pane), never silently dropped.
pub(super) fn rule_line(width: usize, accent: u8, hidden: usize, arrow: char) -> Line<'static> {
    ...
    if width <= tag_w {
        let muted = Style::default().fg(Color::Indexed(crate::ui::theme::muted_idx()));
        return Line::styled(clip_to(tag.trim(), width), muted);
    }
    Line::from(vec![
    ...
    if state.input.draft.is_empty() {
        ...
        return InputLayout {
            rows: vec![InputRowView {
                // Same unguarded-ellipsis hazard as the HUD row (see draw_footer): a working
                // placeholder (`Queue · Alt+↵ steer · Esc stop · N queued`) plus the `[Nimg]` chip
                // can leave a sub-ellipsis budget on narrow panes — `truncate_str(…, 0, "…")`
                // panics in debug. No placeholder beats no composer: the draft text itself
                // renders through the non-empty path, and the keymap is unaffected.
                shown: if budget < 2 {
                    String::new()
                } else {
                    console::truncate_str(&placeholder_text(state), budget, "…").into_owned()
                },
                start: 0,
                cols: vec![0],
            }],

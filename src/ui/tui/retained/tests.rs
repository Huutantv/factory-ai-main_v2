...
#[test]
fn rule_line_reserves_the_hidden_row_tag() {
    // Wide: dashes + tag + dashes, exactly `width` cells.
    let line = rule_line(40, crate::ui::theme::ACCENT_DIM, 12, '↑');
    assert_eq!(line.width(), 40);
    let text: String = line.spans.iter().map(|s| s.content.as_ref()).collect();
    assert!(text.contains("↑12"), "tag present: {text:?}");
    // Narrow: the tag survives even when no dashes fit.
    let narrow = rule_line(4, crate::ui::theme::ACCENT_DIM, 3, '↓');
    let narrow_text: String = narrow.spans.iter().map(|s| s.content.as_ref()).collect();
    assert!(
        narrow_text.contains("↓3"),
        "tag reserved on narrow panes: {narrow_text:?}"
    );
    assert!(narrow.width() <= 4);
    // Nothing hidden: plain rule, no tag.
    let plain = rule_line(40, crate::ui::theme::ACCENT_DIM, 0, '↑');
    let plain_text: String = plain.spans.iter().map(|s| s.content.as_ref()).collect();
    assert!(
        !plain_text.contains('↑'),
        "no tag when open: {plain_text:?}"
    );
}
...
fn max_input_rows_leaves_the_transcript_room() {
    // `cap` is the caller-supplied layout cap (LayoutConfig::default → 10 here).
    assert_eq!(max_input_rows(40, 10), 10, "capped by MAX_INPUT_TEXT_ROWS");
    assert_eq!(max_input_rows(10, 10), 4, "10 - 3 chrome - 3 transcript");
    assert_eq!(
        max_input_rows(6, 10),
        1,
        "a tiny terminal still gets one row"
    );
    assert_eq!(max_input_rows(1, 10), 1, "never zero");
    assert_eq!(
        max_input_rows(40, 6),
        6,
        "a tighter layout cap wins over spare room"
    );
}
...
#[test]
fn ultimate_recolours_the_input_box_to_warn() {
    // `/ultimate` recolours the input box framing to the reserved warn-yellow so the heightened
    // mode is unmistakable, and toggling back returns it to accent-dim. The state is PUSHED (never
    // read from disk in the draw path, which runs at ~9fps).
    let mut state = AppState::new("intro", "status");
    assert!(!state.ultimate, "accent-dim by default");

    apply_command(&mut state, Command::Ultimate(true));
    assert!(state.ultimate);
    apply_command(&mut state, Command::Ultimate(false));
    assert!(!state.ultimate, "toggling off returns to accent-dim");
    // Guard the palette choice itself: warn-yellow must stay distinct from the default dim, or the
...
fn tool_row_puts_digest_and_time_on_the_line_below() {
    ...
    let row = plain(&render_tool_row(&ev, 60, false));
...
fn tool_row_clips_long_target_to_the_frame_width() {
    ...
    let row = plain(&render_tool_row(&ev, 30, false));
    assert!(
...
#[test]
fn tool_row_compact_shape_is_one_line() {
    // The balanced/dense layouts pass compact=true: `icon name — digest · time` on a single row.
    let ev = ToolEvent {
        seq: 1,
        icon: "⚙".into(),
        name: "file_read".into(),
    ...
    let row = plain(&render_tool_row(&ev, 60, true));
    assert!(!row.contains('\n'), "compact is one line: {row:?}");
    assert!(
        row.contains("file_read — 142 lines · 1.2s"),
        "name + digest + time: {row:?}"
    );
}

#[test]
fn tool_row_clips_long_digest_to_the_frame_width() {
    // The default two-line shape used to let the digest run past the pane edge (only the compact
    // shape clipped). The timestamp must never push content out.
    let ev = ToolEvent {
        seq: 1,
        icon: "⚙".into(),
        name: "shell_run".into(),
        target: "make test".into(),
        digest: "a very long digest line that would overflow a narrow pane".into(),
        state: ToolState::Ok,
    ...
    for line in row.split('\n') {
        assert!(
            ...
            "digest line overflowed: {line:?}"
        );
    }
    assert!(row.contains('…'), "long digest visibly clipped: {row:?}");
    assert!(
        row.contains("2.4s"),
        "the run time survives clipping: {row:?}"
    );
}
...
fn a_draft_taller_than_the_box_stops_growing_and_says_what_is_hidden() {
    ...
    // rows behind it so a capped composer never reads as a truncated one. Numbers below assume the
    // balanced default (6 composer rows, 1-cell transcript pad).
    let mut state = AppState::new("intro", "status");
    ...
    assert_eq!(rows[8], "─".repeat(40), "top rule sits above the text rows");
    assert_eq!(rows[9], "❯ dong so 0");
    assert_eq!(rows[14], "  dong so 5", "6 text rows is the ceiling");
    assert!(
        rows[15].ends_with("↓9 ──"),
...
/// The footer-overlap ghosts (`que6ed`, `att37h`): two frames painted through the SAME terminal,
/// across the working→idle edge, must leave only frame two's footer on screen. A TestBackend has
/// no out-of-band writer, so this cannot reproduce the ghost itself — what it pins is the
/// invariant the run loop's clear-on-footer-change relies on: `footer_height` (computed outside
/// the frame, off `autoresize`'s size) agrees with the rows `draw` actually paints, so a reported
/// change is a real geometry change and a reported equality never hides one.
#[test]
fn footer_height_agrees_with_the_painted_footer_across_the_working_edge() {
    let mut state = AppState::new("intro", "status");
    state.working = true;
    state.input.queued_count = 6;
    state.input.images = 3;
    let h = 16u16;
    // Frame one: working, placeholder + chip on row zero. No row may overrun the pane (the
    // pre-guard build panicked here in debug: `truncate_str(_, 0, "…")` underflow).
    let rows = painted_rows(&mut state, 120, h);
    assert!(
        rows.iter().all(|r| console::measure_text_width(r) <= 120),
        "overflow at 120 cols: {rows:?}"
    );
    assert!(
        rows.iter().any(|r| r.contains("Queue · Alt+")),
        "working placeholder painted: {rows:?}"
    );
    let tall = footer_height(&state, 120, h);
    // Frame two: the turn ends with an empty draft — the composer shrinks back and the placeholder
    // is replaced by the right-aligned key hint.
    state.working = false;
    state.input.queued_count = 0;
    state.input.images = 0;
    let rows = painted_rows(&mut state, 120, h);
    let hint_row = rows
        .iter()
        .find(|r| r.contains("↵ send"))
        .expect("idle hint painted");
    assert!(
        !hint_row.contains("Queue") && !hint_row.contains("que"),
        "frame two's hint row carries no frame-one ghost: {hint_row:?}"
    );
    assert!(
        !rows.iter().any(|r| r.contains("Queue · Alt+")),
        "the working placeholder is gone once idle: {rows:?}"
    );
    let short = footer_height(&state, 120, h);
    // Heights are EQUAL across this edge — the placeholder line swaps in for the draft row. That
    ...
    // A 20-column pane where the chip+meter claim the HUD row and the queued placeholder fights
    // the [Nimg] chip for the composer row: paint, don't panic (`truncate_str` underflow), don't spill.
    state.working = true;
    let rows = painted_rows(&mut state, 20, h);
    assert!(
        rows.iter().all(|r| console::measure_text_width(r) <= 20),
        "overflow at 20 cols: {rows:?}"
    );
}

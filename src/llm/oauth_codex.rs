...
async fn respond(sock: &mut tokio::net::TcpStream, message: &str) {
    let body = format!(
        "<!doctype html><meta charset=utf-8><title>F.Auto</title>         <body style=\"font-family:system-ui;padding:2rem\"><h1>F.Auto</h1><p>{message}</p>         <p>You can close this window.</p></body>"
    );
    let resp = format!(
...
pub async fn login_interactive() -> Result<CodexTokenSet> {
    ...
    respond(
        &mut sock,
        "Sign-in complete. Return to the terminal — F.Auto is saving tokens.",
    )
    .await;
...
pub async fn bearer_token() -> Result<(String, Option<String>)> {
    ...
    let mut set = load_token().context(
        "no Codex login — run `fauto auth login codex` (experimental ChatGPT/Codex OAuth)",
    )?;
    if set.is_expired() {
        match refresh_token(&set).await {
            RefreshOutcome::Ok(s) => set = s,
            RefreshOutcome::ReauthRequired(m) => {
                bail!("Codex re-login required: {m} — run `fauto auth login codex`")
            }
...
/// Human status lines for `fauto auth status` (never prints raw tokens).
...
pub fn risk_notice() -> &'static str {
    "EXPERIMENTAL. Uses ChatGPT/Codex consumer OAuth and private backend APIs (not the OpenAI Platform API). May break without notice; may conflict with OpenAI terms; rate limits follow your ChatGPT/Codex plan. Prefer OpenAI API keys or OpenRouter for supported use. Remove tokens with: fauto auth logout codex"
}

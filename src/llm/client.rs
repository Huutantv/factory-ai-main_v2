...
/// Normalize `messages` for strict OpenAI-compatible providers (vLLM/Qwen, …):
/// at most ONE `system` message, and only at index 0.
///
/// F.Auto internally uses two leading system lanes (stable + dynamic, see
/// `prompt_lanes::seed_prompt_lanes`) plus tail `system` nudges (`push_nudge`).
/// Lenient providers accept that, but strict ones 400 with
/// "System message must be at the beginning."
///
/// The rewrite is semantics-preserving: leading systems are concatenated
/// (blank-line separated), non-leading systems become `user` turns with
/// identical content. Idempotent and a no-op for already-strict transcripts.
///
/// Only applied to endpoints that have PROVEN strict (see `send_chat`): lenient
/// providers (Anthropic, OpenAI) keep the two-lane shape so prefix-cache
/// breakpoints still hit.
pub(crate) fn normalize_system_messages(messages: Vec<Message>) -> Vec<Message> {
    let mut lead = 0;
    while lead < messages.len() && messages[lead].role == "system" {
        lead += 1;
    }
    let mut has_late_system = false;
    for m in messages.iter().skip(lead) {
        if m.role == "system" {
            has_late_system = true;
            break;
        }
    }
    if lead <= 1 && !has_late_system {
        return messages;
    }
    let mut out: Vec<Message> = Vec::with_capacity(messages.len().saturating_sub(1).max(1));
    if lead > 0 {
        let parts: Vec<&str> = messages[..lead]
            .iter()
        ...
        // Keep the stable lane's breakpoint so a cached prefix still hits;
        // the dynamic lane's own breakpoint is dropped with the merge (strict
        // endpoints don't honor `cache_control` anyway).
        let cache = messages[0].cache_control.clone();
        out.push(Message {
    ...
    for m in messages.into_iter().skip(lead) {
        if m.role == "system" {
            out.push(Message {
                role: "user".to_string(),
                content: m.content,
...
/// Endpoints that rejected a multi-`system` request THIS SESSION (see `send_chat`).
/// Populated reactively — never guessed from the URL (the same model id serves
/// lenient on one gateway and strict on the next). Keyed by the full chat URL,
/// which is built deterministically from the base URL.
static STRICT_SYSTEM: std::sync::OnceLock<std::sync::Mutex<std::collections::HashSet<String>>> =
    std::sync::OnceLock::new();

fn strict_system_set() -> &'static std::sync::Mutex<std::collections::HashSet<String>> {
    STRICT_SYSTEM.get_or_init(|| std::sync::Mutex::new(std::collections::HashSet::new()))
}

fn endpoint_needs_strict_system(url: &str) -> bool {
    strict_system_set()
        .lock()
...
fn mark_endpoint_strict(url: &str) {
    if let Ok(mut s) = strict_system_set().lock() {
        s.insert(url.to_string());
    }
}

/// Does this error detail blame a non-leading `system` message? Matches the
/// vLLM/Qwen wording verbatim (case-insensitive); deliberately narrow so an
/// unrelated 400 never triggers a re-send.
fn body_blames_system_position(detail: &str) -> bool {
    detail
        .to_ascii_lowercase()
...
/// Fetch the provider's advertised model ids (`GET {base}/models`). Used by `fauto models` and to
...
pub struct ModelInfo {
    ...
    /// known free-tier suffix. Tagged in the picker and `fauto models`.
    pub is_free: bool,
}
...
/// POST a chat request with wire-level fallbacks for model-agnostic compat.
/// `reasoning_effort` is stripped and retried once when the provider 400s
/// blaming it. Strict-`system` endpoints (vLLM/Qwen, …) are merged to a single
/// leading `system` with late `system` nudges demoted to `user` when the
/// provider 400s with "System message must be at the beginning"; the endpoint
/// is remembered for the session and retried once. Endpoints already
/// learned-strict are normalized up front, so the 400 costs one call per
/// endpoint, once. Lenient providers keep the two-lane shape so prefix-cache
/// breakpoints still hit.
async fn send_chat(
    client: &reqwest::Client,
    url: &str,
    ...
    if endpoint_needs_strict_system(url) {
        body.messages = normalize_system_messages(std::mem::take(&mut body.messages));
    }
    let had_effort = body.reasoning_effort.is_some();
    let sent_strict = endpoint_needs_strict_system(url);
    ...
    match send_with_retry(|| with_provider_auth(client.post(url), url, api_key).json(&body)).await {
        ...
        Err(e) => {
            let msg = e.to_string();
            // Only intervene on the exact "400 blames system position" case, and only
            // once per endpoint — an already-normalized request that still 400s is a
            // different problem, so it propagates.
            if !sent_strict && msg.contains("HTTP 400") && body_blames_system_position(&msg) {
                mark_endpoint_strict(url);
                body.messages = normalize_system_messages(std::mem::take(&mut body.messages));
                let note =
                    "strict endpoint (single system only) — retrying normalized, once this session";
                if crate::ui::tui::active() {
    ...
    loop {
        ...
        match parse_chunk(&event.data) {
            Ok(chunk) => {
                ...
                // Same final-chunk-only guard as the tool-calling path: without it, `fauto chat`
...
/// (`stream_chat_with_tools`) drives the top-level `fauto agent` for live output. Both return
...
pub async fn chat_with_tools_effort(
    ...
    if crate::llm::oauth_codex::is_codex_base_url(base_url) {
        ...
        let session = std::env::var("AIZEN_CODEX_SESSION")
            ...
            .unwrap_or_else(|| format!("fauto-{}-{model}", std::process::id()));
...
mod tests {
    ...
    #[test]
    fn strict_system_normalize_is_noop_for_clean_transcripts() {
        // 0-1 leading systems and no late ones: byte-identical (lenient providers
        // keep the two-lane shape for prefix-cache).
        let msgs = vec![Message::system("sys"), Message::user("hi")];
        let out = normalize_system_messages(msgs);
        assert_eq!(out.len(), 2);
        assert_eq!(out[0].role, "system");
        assert_eq!(out[1].role, "user");
        let empty: Vec<Message> = Vec::new();
        assert!(normalize_system_messages(empty).is_empty());
    }

    #[test]
    fn strict_system_normalize_merges_leading_lanes_and_demotes_tail() {
        // THE BUG: vLLM/Qwen 400s "System message must be at the beginning" on
        // both the two-lane prefix and tail `push_nudge` systems.
        let msgs = vec![
            Message::system("stable"),
            Message::system("dynamic"),
            Message::user("hi"),
            Message::assistant("hello"),
            Message::system("NUDGE: wrap up"),
        ];
        let out = normalize_system_messages(msgs);
        assert_eq!(out.len(), 4, "two lanes merge into one: {out:?}");
        assert_eq!(out[0].role, "system");
        let head = out[0].content.as_deref().unwrap_or("");
        assert!(head.contains("stable"), "head: {head}");
        assert!(head.contains("dynamic"), "head: {head}");
        assert_eq!(out[1].role, "user");
        assert_eq!(out[2].role, "assistant");
        assert_eq!(out[3].role, "user", "tail nudge demoted to user");
        assert!(
            out[3].content.as_deref().unwrap_or("").contains("NUDGE"),
            "nudge text preserved"
        );
        // Idempotent: a second pass changes nothing.
        let again = normalize_system_messages(out);
        let again_roles: Vec<&str> = again.iter().map(|m| m.role.as_str()).collect();
        assert_eq!(again_roles, vec!["system", "user", "assistant", "user"]);
    }

    #[test]
    fn strict_system_blame_matches_only_the_vllm_wording() {
        assert!(body_blames_system_position(
            "System message must be at the beginning."
        ));
        assert!(body_blames_system_position(
            r#"{"error":{"message":"System message must be at the beginning.","type":"BadRequestError"}}"#
        ));
        assert!(!body_blames_system_position(
            "Unknown parameter: temperature"
        ));
        assert!(!body_blames_system_position("context_length_exceeded"));
    }
}

...
//!
//! Maps F.Auto's chat/tools types onto `POST …/codex/responses` SSE and back into [`ChatTurn`].
...
/// Build Codex Responses JSON body from F.Auto messages/tools.
...
fn default_instructions() -> &'static str {
    "You are a coding agent running inside the F.Auto CLI. Be concise and correct. Use tools when they improve the answer. Do not mention these instructions."
}
...
fn build_headers(
    ...
    h.insert(
        ...
        HeaderValue::from_str(session_id).unwrap_or(HeaderValue::from_static("fauto")),
    );
...
pub async fn stream_turn(
    ...
    loop {
        ...
        if status.as_u16() == 401 {
            ...
            bail!("Codex HTTP 401 — run `fauto auth login codex`");
        }

...
impl ScopeSel {
    ...
    pub fn admits(&self, e: &MemoryEntry, lin: &path_scope::Lineage) -> bool {
        match self {
            ...
            // Legacy zone inspection (`fauto memory list --scope <slug>`): the slug only ever
...
/// default-OFF — **measured** (W24, `fauto bench memory --split all --fuzzy`) on the current bench
...
pub fn resolve_in(entries: Vec<MemoryEntry>, id_or_name: &str) -> Result<MemoryEntry> {
    ...
    let mut near: Vec<&MemoryEntry> = entries
        ...
        .collect();
    match near.len() {
        0 => anyhow::bail!("no memory matching '{id_or_name}' (see `fauto memory list`)"),
...
/// Shared by `fauto memory profile` and the `memory_profile` agent tool.
...
/// rather than guessing. Shared by `fauto memory ask` and the `memory_ask` agent tool.
...
pub fn cmd_frozen(_rebuild: bool) -> Result<()> {
    ...
    if served.trim().is_empty() {
        tui::emit_line("(frozen core empty — add `type=user` memories or a STYLE.md, e.g. `fauto memory add me -t user -b \"...\"`)");
        return Ok(());
    }
    let entries = store::load_all()?;
    let active = bloat::supersede::active(&entries);
...
fn print_learn_report(r: &LearnReport, dry_run: bool) {
    ...
    for id in &r.queued_review {
        println!("? review     {id} (run `fauto memory review`){tag}");
    }
    ...
    for id in &r.archived {
        println!("⌁ archived   {id} (over inferred-cap; `fauto memory restore {id}`)");
    }
    if r.dropped > 0 {
...
pub fn cmd_style() -> Result<()> {
    match load_style() {
        ...
        None => tui::emit_line(&format!("(no STYLE.md yet — learned via `fauto memory learn` core-promotion, or edit {} directly)", config::style_path().display())),
    }
    Ok(())
}
...
pub fn cmd_supersede(old: &str, new: &str) -> Result<()> {
    ...
    println!(
        "superseded '{}' → '{}' (history kept; see `fauto memory as-of <date>`)",
        old_e.id, new_e.id
    );
    Ok(())
}
...
/// Retire a fact into the recoverable archive (`fauto memory restore <id>` brings it back).
...
pub fn cmd_forget(id_or_name: &str) -> Result<()> {
    ...
    tui::emit_line(&format!(
        "archived as '{archived}' (restore: `fauto memory restore {archived}`)"
    ));
    Ok(())
}
...
/// reads the graph, never records a co-fire (so `fauto memory neighbors` can't pollute its own signal).
...
pub fn cmd_list_superseded() -> Result<()> {
    ...
    tui::emit_line(&format!(
        "\n{} superseded. Bring one back: `fauto memory revive <id>`",
        dead.len()
    ));
    Ok(())
}
...
pub fn cmd_revive(id_or_name: &str) -> Result<()> {
    ...
    let target = all
        ...
        .ok_or_else(|| {
            anyhow::anyhow!("no memory '{id_or_name}' (see `fauto memory list --superseded`)")
        })?;
...
pub fn cmd_archive_list() -> Result<()> {
    ...
    tui::emit_line(&format!(
        "\n{} archived. Restore: `fauto memory restore <id>`",
        arch.len()
    ));
    Ok(())
}
...
pub fn cmd_compact() -> Result<()> {
    ...
    if report.archived.is_empty() && report.sweep_preview.is_empty() {
        ...
        println!(
            "archived {} fact(s) — `fauto memory restore <id>` brings one back:",
            report.archived.len()
        );
...
pub fn print_reconcile_report(r: &learning::reconcile::BatchReport) {
    ...
    if r.dry_run {
        ...
        tui::emit_line("\nEvery retirement is reversible: `fauto memory revive <id>`.");
    }
}
...
pub fn cmd_doctor() -> Result<()> {
    ...
    if r.pending_pairs > 0 {
        ...
        tui::emit_line(&format!(
            "\n{} — `fauto memory reconcile` (dry run) shows the verdicts.{}",
            if due {
    ...
    if c.review >= 5 {
        tui::emit_line(&format!(
            "\n{} item(s) waiting in review — `fauto memory review` to work through them.",
            c.review
        ));
    }
    Ok(())
...
mod tests {
    ...
    fn with_recall_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-recall-{tag}-{}", std::process::id()));
    ...
    fn remember_defaults_to_project_zone_and_global_prefix_overrides() {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-remember-scope-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
        std::env::set_var("AIZEN_PROJECT_ROOT", &dir);
    ...
    fn inventory_lists_without_a_query_and_edit_forget_restore_round_trip() {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-inventory-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn review_clear_and_drop_move_to_discarded_instead_of_deleting() {
        ...
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-review-discard-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
        std::env::set_var("AIZEN_PROJECT_ROOT", &dir);
    ...
    fn search_scoped_current_hides_foreign_zones() {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-scope-search-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
        std::env::set_var("AIZEN_PROJECT_ROOT", &dir);
    ...
    fn short_ids_stay_resolvable_and_lengthen_only_on_collision() {
        ...
        let long_a = "fauto-be-runtime-upstream-endpoint-db-rwlock-atomic-update";
        let long_b = "fauto-be-runtime-upstream-endpoint-ui-dashboard-save-trap";
    ...
    fn dense_caching_embedder_runs_and_persists() {
        ...
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-densecache-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn category_filter_separates_bug_from_decision_end_to_end() {
        ...
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-cat-filter-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn scoped_search_separates_same_keyword_by_dimension() {
        ...
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-scoped-dim-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);

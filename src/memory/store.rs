...
/// `Lineage::specificity`), never deleted, and listed by `fauto memory doctor`. Fail-closed:
...
/// `by_id`) WITHOUT deleting it — the history stays queryable via `fauto memory as-of`.
///
...
/// Soft-delete: move the entry into the recoverable archive (`fauto memory restore <id>` brings it
...
mod tests {
    ...
    fn scope_round_trips_and_survives_reinforce_and_supersede() {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-scope-rt-{}", std::process::id()));
    ...
    fn update_patches_named_fields_and_preserves_the_rest() {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-update-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn retire_is_recoverable_and_purge_is_not() {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-retire-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn unsupersede_restores_a_fact_to_the_active_view() {
        ...
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-unsup-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn record_retrieval_dedups_per_day_and_bumps_reinforced() {
        let _g = config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-reuse-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        std::env::set_var("AIZEN_HOME", &dir);

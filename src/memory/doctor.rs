//! `fauto memory doctor` — read-only health report for the two-axis store.
//!
    ...
    pub fn describe(&self) -> String {
        match self {
            Finding::OrphanPlace { id } => format!(
                "{id}: place fact with no anchor — no directory will ever match it (fix: `fauto memory edit {id}` or re-learn it here)"
            ),
            Finding::DanglingSupersededBy { id, missing } => format!(
                "{id}: retired by '{missing}', which is not in the store — nothing explains why it is hidden (fix: `fauto memory revive {id}`)"
            ),
            ...
            Finding::LiveNearDuplicate { a, b, similarity } => format!(
                "{a} ~ {b}: two live facts at similarity {similarity:.2} — `fauto memory reconcile` judges pairs like this"
            ),
        }
    }
}

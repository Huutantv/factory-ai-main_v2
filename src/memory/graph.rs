...
/// tier). When off, the graph is still built (and queryable via `fauto memory neighbors`), it just
...
mod tests {
    ...
    fn with_temp_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-graph-{tag}-{}", std::process::id()));

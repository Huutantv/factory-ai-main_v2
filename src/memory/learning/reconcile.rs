...
/// answer backwards: `tokio 1.35` anchored at `…/fauto/src` would beat `tokio 1.40` anchored at
/// `…/fauto` purely for being deeper, i.e. the stale fact wins for having been written about a
...
///   fact — so `fauto memory revive <id>` undoes it, `memory as-of` still shows it, and the row that
...
mod tests {
    ...
    fn refine_resets_confirmations() {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-refine-{}", std::process::id()));
    ...
    fn dropping_a_live_duplicate_hides_it_reversibly_without_deleting_the_file() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-dupe-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn dropping_a_queued_duplicate_moves_it_aside_rather_than_retiring_it() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-dupeq-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn a_dry_run_removes_nothing() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-dry-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn applying_a_whole_cluster_leaves_exactly_one_live_row() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-cluster-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::env::set_var("AIZEN_HOME", &dir);
    ...
    fn newer_wins_over_deeper_on_same_chain() {
        ...
        // repo root.
        let deep_old = place("tokio-old", "tokio 1.35", "c:/work/fauto/src", "2026-01-01");
        let shallow_new = place("tokio-new", "tokio 1.40", "c:/work/fauto", "2026-07-01");
        ...
        assert_eq!(w.reanchor.as_deref(), Some("c:/work/fauto"));
        ...
        let a = place("a", "tokio 1.35", "c:/work/fauto/src", "2026-07-01");
        let b = place("b", "tokio 1.40", "c:/work/fauto", "2026-07-01");
        assert_eq!(

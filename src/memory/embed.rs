...
/// auto-detected, not loaded from the configured name, and can see WHICH dir fauto chose.
#[derive(Debug, Clone)]
pub struct ModelCandidate {
    pub name: String,
    pub dir: PathBuf,
    /// "fauto" = under `~/.aizen/models/<name>/`; "hf" = under the HF hub cache snapshot dir.
...
/// 2. Any model2vec model under `~/.aizen/models/` — the fauto-managed dir.
...
pub fn discover_local_model() -> Option<ModelCandidate> {
    ...
    // 2–3. Scan the fauto models dir, then the HF hub cache, and take the best-ranked candidate.
    list_local_models().into_iter().next()
}

/// Every model2vec model this machine has, best-ranked first — the listing behind
/// `fauto memory model-list`, and the candidate pool [`discover_local_model`] picks from.
///
...
/// fauto-managed dir is scanned before the HF cache, so an fauto copy wins a same-name tie.
pub fn list_local_models() -> Vec<ModelCandidate> {
    let mut all = scan_dir_for_model2vec(&config::models_dir(), "fauto");
...
pub mod model2vec {
    ...
    pub struct Model2VecEmbedder {
        ...
        name: String,
        dim: usize,
        /// "path" / "fauto" / "hf" — where the model was loaded from, so the note can say it was
    ...
    impl Model2VecEmbedder {
        ...
        pub fn load_default() -> Result<Self> {
            let Some(cand) = super::discover_local_model() else {
                anyhow::bail!(
                    "dense model '{}' not found, and no model2vec model in ~/.aizen/models or the HF cache — run `fauto memory model-download`",
    ...
    impl Model2VecEmbedder {
        /// Where the loaded model came from (`"fauto"` / `"hf"`), for the boot note.
...
mod tests {
    ...
    fn discover_picks_configured_name_when_present() {
        let tmp = std::env::temp_dir().join(format!("fauto-disc-cfg-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        let models = fauto.join("models"); // models_dir() = AIZEN_HOME/models
                                           // Configured default is potion-base-8M; put it AND a less-preferred model in the fauto dir.
        write_model2vec_dir(&models.join("potion-base-8M"), "potion-base-8M");
        write_model2vec_dir(&models.join("zzz-other"), "zzz-other");
        let _env = IsolatedModelEnv::pin(&fauto, &hf);
        let cand = discover_local_model().expect("configured model should win");
        assert_eq!(cand.name, "potion-base-8M");
        assert_eq!(cand.source, "fauto");
    }

    #[test]
    fn discover_falls_back_to_a_neighbor_when_configured_missing() {
        let tmp = std::env::temp_dir().join(format!("fauto-disc-nb-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        let models = fauto.join("models");
    ...
    fn discover_skips_non_model2vec_dirs() {
        let tmp = std::env::temp_dir().join(format!("fauto-disc-skip-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        let models = fauto.join("models");
    ...
    fn discover_finds_model_in_hf_cache_snapshot() {
        let tmp = std::env::temp_dir().join(format!("fauto-disc-hf-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        // Empty fauto dir; a model2vec model sits in the HF cache under the standard layout.
        std::fs::create_dir_all(&fauto).unwrap();
    ...
    fn discover_returns_none_when_nothing_usable_anywhere() {
        let tmp = std::env::temp_dir().join(format!("fauto-disc-empty-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        std::fs::create_dir_all(&fauto).unwrap();
    ...
    fn absolute_path_in_embed_model_is_used_directly() {
        ...
        let tmp = std::env::temp_dir().join(format!("fauto-disc-abs-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        ...
        write_model2vec_dir(&fauto.join("models/potion-base-8M"), "potion-base-8M");
        write_model2vec_dir(&elsewhere, "my-model");
        let env = IsolatedModelEnv::pin(&fauto, &hf);
    ...
    fn absolute_path_that_is_not_a_model_finds_nothing() {
        ...
        let tmp = std::env::temp_dir().join(format!("fauto-disc-abs-bad-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let fauto = tmp.join("fauto");
        let hf = tmp.join("hf");
        std::fs::create_dir_all(&fauto).unwrap();
    ...
    fn hf_cache_is_found_via_each_documented_root() {
        ...
        {
            let tmp =
                std::env::temp_dir().join(format!("fauto-disc-root-{}-{idx}", std::process::id()));
            let _ = std::fs::remove_dir_all(&tmp);
            let fauto = tmp.join("fauto");
            std::fs::create_dir_all(&fauto).unwrap();
            ...
            let _env = IsolatedModelEnv::pin(&fauto, &tmp.join("unused-hub"));
    ...
    fn is_model2vec_dir_requires_the_signature() {
        let tmp = std::env::temp_dir().join(format!("fauto-disc-sig-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);

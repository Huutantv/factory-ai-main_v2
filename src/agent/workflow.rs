//! `fauto workflow <spec.json>` — the lean orchestration layer (fan-out + mixture-of-agents synth).
//!
...
/// Emit the workflow header line (the fan-out banner) into the sticky-TUI transcript — an accent
...
fn wf_header(line: &str) {
    if crate::ui::tui::active() {
        let star = console::style("✦")
            .color256(crate::ui::theme::ACCENT)
            .bold();
...
mod tests {
    ...
    async fn collect_resolves_paths_against_the_supplied_root_not_the_process_cwd() {
        // The model-callable `workflow` used to read `current_dir()`, so under `fauto serve` — where
        ...
        let lane_root = std::env::temp_dir().join("fauto-wf-root-probe");

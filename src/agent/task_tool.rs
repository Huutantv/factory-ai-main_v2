            ...
            Err(SlotDenied::Io(e)) => {
                return Ok(format!(
                    "error: the sub-agent gate is unavailable on this machine ({e}) — this is NOT \
                     a concurrency limit and retrying will not help; tell the user their fauto \
...
impl SubagentSlot {
    pub(crate) fn try_acquire() -> Result<Self, SlotDenied> {
        ...
        // Probe the whole GLOBAL band, not `0..cap`. The slot files are shared by every fauto
...
mod tests {
    ...
    fn writers_denylist_covers_every_destructive_role_tool() {
        ...
        // known OUTWARD tool (side effects land outside the workspace — fauto's own stores,
    ...
    fn resolve_dispatch_agent_beats_role_and_falls_back() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let sandbox = std::env::temp_dir().join(format!("fauto-disp-{}", std::process::id()));
    ...
    fn resolve_dispatch_routes_endpoint_by_model() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let sandbox = std::env::temp_dir().join(format!("fauto-disp-ep-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&sandbox);
        // RESTORE on drop — see `EnvGuard`: deleting USERPROFILE/HOME disables
        // home-boundary guards for whatever test runs next.
        let _env = crate::core::config::EnvGuard::set([
    ...
    fn specialist_provider_route_keeps_provider_when_model_is_overridden() {
        let _g = crate::core::config::TEST_HOME_LOCK
            ...
            .unwrap_or_else(|e| e.into_inner());
        let sandbox =
            std::env::temp_dir().join(format!("fauto-agent-provider-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&sandbox);
        let _env = crate::core::config::EnvGuard::set([
            ("USERPROFILE", sandbox.clone()),
            ("HOME", sandbox.clone()),
    ...
    fn card_endpoint_beats_model_registry() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let sandbox = std::env::temp_dir().join(format!("fauto-card-ep-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&sandbox);

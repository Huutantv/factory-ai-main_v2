//! Pure-Rust **OAuth 2.1 (PKCE)** client for MCP remote servers — the piece that lets `fauto apps`
...
async fn respond(sock: &mut tokio::net::TcpStream, message: &str) {
    let body = format!(
        "<!doctype html><meta charset=utf-8><title>F.Auto</title>\
         <body style=\"font-family:system-ui;background:#0b0b0c;color:#eee;display:grid;place-items:center;height:100vh;margin:0\">\
         <div style=\"text-align:center\"><h2 style=\"color:#e3b341\">F.Auto</h2><p>{message}</p></div>"
    );
    let resp = format!(
...
async fn wait_for_code(listener: tokio::net::TcpListener, expected_state: &str) -> Result<String> {
    ...
    loop {
        ...
        respond(
            &mut sock,
            "Signed in. You can close this tab and return to F.Auto.",
        )
        .await;
...
pub async fn refresh(key: &str, t: &TokenSet) -> Result<TokenSet> {
    let rt = t
        .refresh_token
        .clone()
        .context("no refresh token — run `fauto apps login` again")?;
    let client = http_client()?;

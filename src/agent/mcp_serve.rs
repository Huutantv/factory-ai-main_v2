//! `fauto mcp serve` — fauto as an MCP **server**, so another agent can hand it work.
//!
...
//! — anything that speaks MCP — spawns `fauto mcp serve` in a repository and gets three tools:
//! dispatch one specialist, fan several out and synthesise them, and ask who is available.
//!
//! Why it is worth having, in one line: **fauto's specialists become callable by an agent that is
//! not fauto.** A card you wrote once in `~/.aizen/agents` — your reviewer, with your rules, on your
//! model — can now be dispatched by whatever assistant you happen to be talking to, without that
//! assistant knowing anything about fauto beyond the tool schema.
//!
...
//!   one started by fauto itself — stamped with who asked, from the MCP handshake.
...
fn initialize(params: &Value) -> Value {
    ...
    json!({
        ...
        "capabilities": {"tools": {}},
        "serverInfo": {"name": "fauto", "version": env!("CARGO_PKG_VERSION")},
    })
}
...
fn tools(registry: &ToolRegistry, allow_write: bool) -> Vec<Value> {
    ...
    for name in ADVERTISED {
        ...
        if !allow_write {
            description.push_str(
                "\n\nREAD-ONLY SERVER: this fauto was started without --yes, so any dispatch that \
    ...
    out.push(json!({
        "name": ROSTER,
        "description":
            "List the fauto specialists available on this machine: slug to pass as `agent`, what \
    ...
    let refusal = |what: &str| {
        Some(format!(
            "từ chối: {what} sẽ chạy với quyền sửa tệp hoặc chạy lệnh, mà máy chủ này khởi động \
             ...
             `fauto mcp serve --yes`. Quyền đó không nằm trong tham số bạn gửi.",
        ))
    };
    match name {
...
fn roster() -> String {
    ...
    if lines.is_empty() {
        return "Chưa có specialist nào trên máy này. Dùng role planner/reviewer/coder/tester, \
                hoặc tạo thẻ bằng `fauto agents install`."
            .to_string();
    }
    format!(
...
mod tests {
    ...
    fn the_handshake_names_the_caller() {
        ...
        assert_eq!(v["result"]["serverInfo"]["name"], "fauto");

...
/// Catalog order matches Hermes CLI bundles where fauto has an analogue (+ `mcp`, `lsp`).
...
pub fn format_status(registry: &ToolRegistry) -> String {
    ...
    out.push_str(
        "\nconfig: `fauto config set --disabled-toolsets web,browser` or /tools in the REPL\n",
    );

...
/// `strings fauto(.exe)` can't lift it. `decode` reverses the build-time XOR; the public accessors
...
/// the opencode precedent (`qwen.txt` fallback) applied to fauto's arbitrary-endpoint reality.
...
pub fn build_top_level_system_prompt_bundle(
    ...
    // Ultimate mode (fauto's `ultracode`): a pure SUFFIX telling the model to reason at max depth and
...
pub struct AgentOutcome {
    /// The model's final answer (already streamed to stdout by `fauto agent`; consumed by the
...
/// text) compresses older turns in place. For multi-turn callers (e.g. `fauto serve`) whose sessions
    ...
    loop {
        ...
        if !goal_mode && iter >= cap {
            ...
            if should_auto_extend(
                ...
                // looping — it is simply a big task still moving. Cutting it here is what made fauto
                ...
                if let Some(p) = u.prompt_tokens {
                    ...
                    let line = format!(
                        ...
                        if crate::ui::icons::on() {
                            crate::ui::icons::g(crate::ui::icons::cost())
                        } else {
                            String::new()
                        },
                        fmt_tok(p),
                    ...
                    Some(tool) => match gate_and_approve(tool.as_ref(), args, cfg) {
                        ...
                        None => {
                            ...
                            let lease_error = if writer_lease.is_none()
                                ...
                                let cwd = cfg.effective_root();
                                // 15s, not 5: transient contention (another fauto instance, an
                                ...
                                match crate::features::timemachine::save_protected_change_in(crate::features::timemachine::PRE_EDIT_LABEL, target_dir.as_deref()) {
                                    ...
                                    Ok(Some(snap)) => {
                                        *auto_checkpointed = true;
                                        ...
                                        if !cfg.quiet {
                                            emit_trace(&format!(
                                                "→ checkpoint #{} saved (agent: `checkpoint` action=rewind target=pre_edit; human: `fauto time restore {}`)",
                                                snap.id, snap.id
                                            ));
                                        }
                                        None
...
/// it reads `◆ <verb + target> (tool_name)` — the verb+target in accent, the raw tool name
...
/// The `◆` anchor is accent here (the call is starting); the result corner `└` on the
/// A one-shot styled string form of the call line (mockup shape `⚙ <name>   <target>`) — the raw
/// tool name in accent, its salient target dimmed. Used for the APPROVAL PROMPT (a single
...
fn stamp_phase_checkpoint(quiet: bool, label: &str) {
    match crate::features::timemachine::save(label, true) {
        Ok(snap) => {
            ...
            if !quiet {
                emit_trace(&format!(
                    "  └ checkpoint #{} — {label} (agent: `checkpoint` action=rewind target=last_good; human: `fauto time restore {}`)",
                    snap.id, snap.id
                ));
            }
        }
...
/// The tool-call anchor icon — one glyph grouped by what the tool touches ([`icons::tool_kind`]),
/// or the plain diamond when icons are off. Kept plain (no SGR) so the retained/classic renderer
/// tints it as part of the line.
fn tool_icon(name: &str) -> &'static str {
    if crate::ui::icons::on() {
        crate::ui::icons::tool_kind(name)
    } else {
        "◆"
    }
}
...
fn emit_tool_call(name: &str, args: &serde_json::Value) -> u64 {
    ...
    crate::ui::tui::tool_call_begin(tool_icon(name), name, &tool_target(name, args))
}
...
/// `fauto serve` daemon (non-TTY but Telegram-connected), route the approval to the owner's phone
...
mod tests {
    ...
    async fn a_progressing_run_is_continued_past_the_step_cap_instead_of_cut_off() {
        // THE regression this exists for: fauto stopping mid-task. A run that keeps producing NEW
    ...
    fn obfuscated_prompts_decode_correctly() {
        ...
        assert!(
            base.to_lowercase().contains("fauto"),
            "decoded base prompt mentions fauto"
        );
    ...
    async fn identical_re_read_short_circuits_only_while_unchanged() {
        let dir = std::env::temp_dir().join(format!("fauto-reread-{}", std::process::id()));
    ...
    async fn batch_coach_nudges_once_after_a_single_read_streak() {
        let dir = std::env::temp_dir().join(format!("fauto-batchcoach-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn with_agent_sandbox<T>(tag: &str, f: impl FnOnce(&std::path::Path) -> T) -> T {
        ...
        let root = std::env::temp_dir().join(format!("fauto-tlp-{tag}-{}", std::process::id()));

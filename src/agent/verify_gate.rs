...
pub fn format_gate_failure(r: &VerifyGateResult) -> String {
    if !r.stable {
        return format!(
            "[fauto verify] `{}` was discarded because the source workspace changed while it ran. {}",
            r.command, r.output
        );
    }
    let mut msg = format!(
        "[fauto verify] `{}` FAILED ({} ms). Fix these errors before reporting the task done:\n\n{}",
        r.command,
        r.duration_ms,
...
pub fn format_delta_failure(result: &VerifyGateResult, delta: &VerifyDelta) -> String {
    ...
    format!(
        "[fauto verify] `{}` introduced {} NEW diagnostic(s) ({} ms). Fix only these regressions before reporting the task done:\n\n{}",
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        let d = std::env::temp_dir().join(format!("fauto-verify-{tag}-{}", std::process::id()));

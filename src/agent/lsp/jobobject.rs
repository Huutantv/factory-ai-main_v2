...
//! RAM) after F.Auto exits or `/lsp off`. The fix (the standard one, used by every serious Windows
...
//! F.Auto process dies and the OS closes its handles), the kernel terminates every process in the

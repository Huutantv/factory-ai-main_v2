...
/// `fauto skill` path are unaffected.
...
impl Tool for MemoryUpdate {
    ...
    fn execute(&self, args: &Value) -> Result<String> {
        ...
        let patch = crate::memory::store::EntryPatch {
            ...
            // is `fauto memory revive` — a human decision with its own audit line, not a field an
    ...
    fn execute(&self, args: &Value) -> Result<String> {
        ...
        match crate::memory::store::retire(&e) {
            Ok(archived) => Ok(format!(
                "retired memory '{}' → archived as '{archived}' (user can restore: `fauto memory restore {archived}`)",
                e.id
            )),
            Err(err) => Ok(format!("error: {err}")),
        }
...
impl Tool for FileWrite {
    ...
    fn execute(&self, args: &Value) -> Result<String> {
        ...
        // lines up, so it passes happily when another fauto window rewrote this file three turns ago:
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("fauto-agent-tool-{tag}-{}", std::process::id()));
    ...
    fn file_glob_fuzzy_finds_parent_folder_by_near_name() {
        // The workspace is often a subdir (e.g. .../mini_project/fauto). Asking for the parent
        ...
        let ws = base.join("mini_project").join("fauto");
    ...
    fn persona_create_writes_and_activates() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("fauto-persona-tool-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);

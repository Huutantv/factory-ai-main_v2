...
//! object / Unix process group), so `kill` reaps the whole tree and — on Windows — an F.Auto crash
...
struct ProcEntry {
    ...
    /// this is also crash insurance — if F.Auto dies without running `kill_all`, the OS closes the job

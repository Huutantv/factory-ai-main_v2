...
//!
//! Claude Code has `/workflows` to watch fan-out progress. F.Auto already ran `task` + `workflow`
...
struct RunManifest {
    ...
    ///
    /// Empty (and omitted from the file) for every ordinary run: a REPL turn, a one-shot `fauto`
    /// invocation, the daemon. It is filled only by [`set_origin`], and today the only caller is
    /// `fauto mcp serve`, which learns the name from the MCP handshake — so a fan-out that Claude
...
/// The short, typeable form of a run id. Ids embed the pid in their high 32 bits (so two fauto
...
pub fn format_status() -> String {
    ...
    if g.live.is_empty() && g.history.is_empty() && load_remote_manifests().is_empty() {
        out.push_str(
            "\n  (no task/workflow activity this process yet)\n\
             ...
             (or `fauto workflow <spec.json>`). Ultimate mode prefers workflows for fan-out.\n\
    ...
    out.push_str(
        "\n\nStop ONE run: `/workflows stop #<id>` (or a name fragment) — Esc cancels the whole turn\n\
         ...
         CLI: `fauto workflow <spec.json>` · specialists: `fauto agents list`",
    );
    out
}
...
mod tests {
    ...
    Two directions, both load-bearing: a manifest written by an older fauto (or by any ordinary

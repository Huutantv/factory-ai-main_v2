//! Import conversations recorded by OTHER CLIs (Claude Code, Codex) and resume them inside fauto.
//!
//! Each CLI keeps its own append-only JSONL transcript on disk. None of them share a schema, and
//! none of them match fauto's `Vec<Message>` wire shape — but they all record the same *thing* (a
...
//! - `thinking` / `reasoning` blocks. fauto strips chain-of-thought from its OWN model's output
...
//!   fauto always discards, and there is no field on `Message` that holds them honestly.
//! - `system` / `developer` turns. The source CLI's harness prompt is not fauto's; leaving it in
...
/// Parse a foreign transcript into fauto's `Vec<Message>` and repair it so it satisfies
...
/// Full Claude CLI parse → fauto messages. Reads every line; drops sidechain, system/meta
...
/// If a Claude user `message` is a tool_result envelope, build the fauto `tool` message from it.
...
/// Build an fauto assistant message from a Claude assistant turn: concatenate `text` parts (skip
        ...
        match part.get("type").and_then(|t| t.as_str()) {
            ...
            Some("tool_use") => {
                ...
                let name = part
                    ...
                    .unwrap_or("")
                    .to_string();
                // `input` is a JSON object; fauto's `FunctionCall.arguments` is the STRINGIFIED
...
/// Full Codex parse → fauto messages. Codex emits `response_item` lines whose payload is a
        ...
        match pty {
            "message" => {
                ...
                match role {
                    ...
                    _ => {} // developer/system harness — dropped (fauto seeds its own prompt).
                }
            }
            "function_call" | "custom_tool_call" => {
                ...
                let name = payload
                    ...
                    .unwrap_or("")
                    .to_string();
                // Codex `arguments` is already a stringified JSON object (like fauto's wire shape).
...
/// satisfies the invariant every fauto request relies on:
///
...
mod tests {
    ...
    fn tmp_home(tag: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("fauto-foreign-{}-{tag}", std::process::id()));
    ...
    /// transcripts would have been offered for the fauto checkout, of which 9 were genuine and 9 were
    ...
    fn cwd_filter_accepts_subdirs_and_rejects_ancestors() {
        let root = std::path::Path::new(r"C:\Users\me\proj\fauto");
        // Exact, including the drive-letter casing Claude and Codex disagree about.
        assert!(cwd_matches(r"C:\Users\me\proj\fauto", root));
        assert!(cwd_matches(r"c:\users\me\proj\fauto", root));
        assert!(
            cwd_matches(r"C:\Users\me\proj\fauto\", root),
            "trailing sep"
        );
        // Launched from a subdir of the checkout — still this project.
        assert!(cwd_matches(r"C:\Users\me\proj\fauto\crates\core", root));
    ...
    /// repair_history satisfies the invariant every fauto request relies on. This is the closest

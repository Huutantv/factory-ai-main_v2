...
//! objects, and any blob that lived only in the source is gone. A future `fauto time seal` (copy the
...
/// destructive edit of every agent run, so they pile up fast (81 on this machine's fauto store) and
        ...
        if g.rewinds_used >= MAX_RUN_REWINDS {
            bail!(
                "run rewind budget exhausted ({MAX_RUN_REWINDS}/{MAX_RUN_REWINDS}). \
                 Further recovery is human-driven: `fauto time list` / `fauto time restore <id>`."
            );
        }
        let id = match target {
...
impl RepoContext {
    ...
    /// Previous hardening location: `<common-git-dir>/fauto-timemachine/<worktree-id>/ledger.json`.
    fn inrepo_namespace_ledger_path(&self) -> PathBuf {
        self.common_git_dir
            .join("fauto-timemachine")
        ...
        S: AsRef<OsStr>,
    {
        ...
        cmd.current_dir(&self.root)
            ...
            .env("GIT_AUTHOR_NAME", "F.Auto Time Machine")
            .env("GIT_AUTHOR_EMAIL", "timemachine@localhost")
            .env("GIT_COMMITTER_NAME", "F.Auto Time Machine")
    ...
    fn load_ledger(&self) -> Result<Ledger> {
        ...
        let mut ledger: Ledger = serde_json::from_slice(&bytes).with_context(|| {
            format!(
                "time-machine ledger {} is corrupt; run `fauto time doctor`",
                path.display()
            )
        })?;
        ledger.normalize()?;
    ...
    fn load_journal(&self) -> Result<Option<Journal>> {
        ...
        let journal = serde_json::from_slice(&bytes).with_context(|| {
            format!(
                "transaction journal {} is corrupt; run `fauto time doctor`",
                path.display()
            )
        })?;
        Ok(Some(journal))
    ...
    fn recover_pending(&self, ledger: &mut Ledger) -> Result<()> {
        ...
        if journal.expected_generation > ledger.generation {
            bail!("time-machine journal belongs to a future ledger generation; run `fauto time doctor`");
        }
        match journal.kind {
            JournalKind::Save => {
                let id = journal
            ...
            JournalKind::Restore => {
                ...
                if let Some(preimage_id) = journal.preimage_id {
                    ...
                    if rolled_back != preimage.tree {
                        bail!("interrupted restore rollback did not reproduce its preimage; run `fauto time doctor`");
                    }
                    ledger.set_cursor(Some(preimage_id));
                    self.save_ledger(ledger)?;
                    self.clear_journal()?;
                ...
                bail!("interrupted restore has no recovery preimage; run `fauto time doctor`");
            }
...
fn run_git_piped_bounded(cmd: &mut Command, stdin_bytes: &[u8], what: &str) -> Result<Output> {
    ...
    if timed_out {
        ...
        bail!(
            "{what} exceeded {}s and was terminated (set AIZEN_GIT_OP_TIMEOUT_SECS to raise the \
             limit); it may have completed partially — run `fauto time doctor` to check the store",
            timeout.as_secs()
        );
    }
...
/// that fauto is broken — the actual fact was that the SESSION's cwd was `C:\Users\admin`, a
...
/// reported a path the user then found ABSENT on disk, with no fauto process running, and deleting
...
pub fn checkpoint_failure_hint(e: &anyhow::Error) -> String {
    ...
    " — this lock is held on an open handle, not by the file on disk, so deleting it has no effect \
     and an absent file does not mean it is free. It is most likely held by THIS fauto session (a \
     stuck internal git call from an earlier turn). Run `fauto time doctor`, or restart this \
...
fn restore_in(ctx: &RepoContext, id: u32) -> Result<Snapshot> {
    ...
    let target = ledger
        ...
        .find(|s| s.id == id)
        .cloned()
        .with_context(|| format!("no checkpoint #{id} (see `fauto time list`)"))?;
    ...
    if actual != target.tree {
        ...
        bail!(
            "restore verification failed; recovery journal was preserved for `fauto time doctor`"
        );
    }
...
pub fn undo() -> Result<Snapshot> {
    ...
    let current = ledger
        ...
        .context("no checkpoints yet — save one with `fauto time save`")?;
    let parent = ledger
        .snapshots
        .iter()
        .find(|s| s.id == current)
...
pub struct DoctorReport {
    ...
    /// thousands of files. `fauto time gc` packs them.
    ...
    fn save_point(&self, args: &serde_json::Value) -> Result<String> {
        ...
        Ok(format!(
            "checkpoint #{} saved ({}). Run-scoped rewind: action=rewind target=last_good|pre_edit. \
             Human free-form: `fauto time restore {}`.",
            snap.id,
            ...
            Ok(snap) => {
                ...
                Ok(format!(
                    "restored working tree to checkpoint #{} ({}).{why} Files only — chat history was NOT \
                     ...
                     `fauto time redo`). Re-read files you care about — contents changed on disk.",
                    snap.id,
                    if snap.label.is_empty() { "no label" } else { &snap.label },
                ))
            }
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("fauto-tm-{tag}-{}", std::process::id()));
    ...
    /// busy …transaction.lock", found no other fauto process and no lock file on disk, and had no
    ...
    fn diff_side_parses_ids_and_working_aliases() {
        ...
        // lookup is what rejects it, with a message naming `fauto time list`.

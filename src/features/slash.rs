...
pub const BUILTINS: &[Builtin] = &[
    ...
    Builtin {
        ...
        argument_hint: "",
        help: "show THIS project's identity: root · zone slug · git executable · where memory/skills/sessions live (also `fauto where`, `fauto zone migrate`)",
        stdin: Stdin::Never,
    },
    Builtin {
        id: SlashId::Handoff,
    ...
    Builtin {
        ...
        help: "the OS sandbox under approval: status shows what THIS machine enforces (kernel vs software); auto|strict|guarded|off sets the mode (strict fails closed where the kernel can't enforce). Full detail: `fauto sandbox status|doctor`",
        stdin: Stdin::Never,
    },
    Builtin {
        id: SlashId::Effort,
    ...
    Builtin {
        ...
        argument_hint: "",
        help: "toggle ultimate mode — max reasoning effort + prefer launching workflows (fauto's ultracode)",
        stdin: Stdin::Never,
    },
    Builtin {
        id: SlashId::Clear,
    ...
    Builtin {
        ...
        argument_hint: "",
        help: "session usage + $ estimate (real tokens when the provider reports them; set rates via `fauto config set --price-in/--price-out`)",
        stdin: Stdin::Never,
    },
    Builtin {
        id: SlashId::Tools,
    ...
    Builtin {
        ...
        hidden_aliases: &[],
        hidden: false,
        description: "show every fauto version and install the one you pick",
        argument_hint: "",
...
Builtin {
    ...
    Builtin {
        ...
        argument_hint: "[default|balanced|dense|list|<name>]",
        help: "bare /layout: arrow-key picker. default/balanced/polished/dense/spacious/auto. Custom layouts live in ~/.aizen/layout/*.{json,jsonc}",
        stdin: Stdin::WhenBare,
    },
];
...
/// `/model của fauto là gì?` is a question about the model, not a request to open the picker.
...
///
/// `/clear now` = 1 → still a command. `/model của fauto là gì?` = 4 → a question.
...
mod tests {
    ...
    fn no_arg_command_with_a_sentence_after_it_is_a_question() {
        ...
        assert_eq!(classify("/model của fauto là gì?"), Verdict::Chat);

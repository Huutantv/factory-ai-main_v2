...
//!
//! F.Auto installs a downloaded release beside the running executable. It never re-execs or kills the
...
async fn list_releases(limit: usize) -> Result<Vec<ReleaseInfo>> {
    let suffix = asset_suffix().ok_or_else(|| {
        anyhow::anyhow!(
            "no published F.Auto asset for this platform ({} / {})",
            std::env::consts::OS,
            std::env::consts::ARCH
        )
    })?;
...
pub fn cleanup_stale_backups(dir: &Path, live_exe: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        if name != live_name && name.starts_with("fauto") && name.contains(".old-") {
...
pub fn cached_notice() -> Option<String> {
    ...
    (cache.for_version == current_version() && is_newer(&cache.latest_tag, current_version()))
        .then(|| format!("⬆ fauto {} is available — run /update", cache.latest_tag))
}
...
///
/// This is the only entry point — `fauto update` and `/update` both land here with no arguments.
/// It owns stdin through dialoguer, so callers must have suspended the retained frame (see
/// `ui::tui::slash_takes_stdin`).
pub async fn run() -> Result<()> {
    let target = std::env::current_exe().context("resolving the fauto executable path")?;
    ...
    emit(&format!(
        "fauto {} · pick a version to install (Esc to cancel)",
        current_version()
    ));
    ...
    if is_running_tag(&selected.tag) {
        emit(&format!(
            "fauto {} is already running — nothing to install",
            current_version()
        ));
        return Ok(());
    }
...
mod tests {
    ...
    fn release_parser_filters_drafts_and_missing_assets() {
        let json = r#"[
          {"tag_name":"v0.5.0","name":"five","draft":false,"prerelease":false,"published_at":"today","assets":[{"name":"fauto-v0.5.0-windows-x86_64.exe","browser_download_url":"https://x/a","size":12}]},
          {"tag_name":"v0.4.9","draft":true,"assets":[{"name":"fauto-v0.4.9-windows-x86_64.exe","browser_download_url":"https://x/b","size":10}]},
    ...
    fn scratch(tag: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!("fauto-update-{}-{tag}", std::process::id()));
    ...
    fn cleanup_removes_stale_backups_but_never_the_live_binary() {
        ...
        let live = root.join("fauto.exe");
        let stale = root.join("fauto.exe.old-0.4.7-123");

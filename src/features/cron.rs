//! Daemon-free scheduled agent tasks (`fauto cron`). Instead of an always-on daemon + embedded
...
//! `fauto cron run <name>` in a FRESH process at the schedule. Job specs live as JSON under
...
fn add(name: &str, schedule: &str, task: &str) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("resolving the fauto executable path")?;
...
fn list() -> Result<()> {
    ...
    if !found {
        println!("(no scheduled jobs — add one with `fauto cron add <name> --schedule daily@09:00 --task \"…\"`)");
    }
    Ok(())
}

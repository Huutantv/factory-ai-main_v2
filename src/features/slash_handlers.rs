...
use crate::ui::{config_ui, icons, theme, tui};
use crate::*;
use anyhow::Result;
use console::style;
...
/// audit and correct it without dropping to the CLI. Sub-commands mirror `fauto memory <sub>` 1:1
    ...
    match sub {
        ...
        "edit" | "update" => {
            ...
            if id.is_empty() || body.is_empty() {
                anyhow::bail!("usage: /memory edit <id> <corrected fact>  (field flags: `fauto memory edit --help`)");
            }
        ...
        // could see them. Same functions as `fauto memory review`, so the two can't drift.
        "review" | "queue" => {
...
async fn slash_init(arg: &str) {
        ...
        match codebase::load() {
            Some(idx) => {
                ...
                tui::emit_line(&format!(
                    ...
                    style("✓ codebase index:").color256(theme::ACCENT),
                    idx.files.len(),
                    idx.chunks.len(),
                    ago
                ));
    ...
    match result {
        Ok(Ok(stats)) => {
            ...
            tui::emit_line(&format!(
                "{} {} in {}ms",
                style("✓ codebase indexed:").color256(theme::ACCENT),
                parts.join(", "),
                stats.elapsed_ms
            ));
...
/// Rows for `/team status` — every fauto session registered in this repository.
///
...
pub(crate) fn team_status_lines() -> Vec<String> {
    ...
    if sessions.is_empty() {
        return vec![style(
            "no fauto sessions registered here yet — this window registers itself on start",
        )
        .dim()
        .to_string()];
    }
    let mut out = vec![format!(
        ...
        style("⚑ team").color256(theme::ACCENT).bold(),
        sessions.len()
    )];
...
/// `/team` — read and act on what the OTHER fauto windows in this repository are doing.
    ...
    match sub {
        ...
        "claims" => {
            ...
            tui::emit_line(&format!(
                ...
                style("⚑ claims").color256(theme::ACCENT).bold(),
                claims.len()
            ));
...
async fn slash_team_commit(rest: &str) {
    ...
    tui::emit_line(&format!(
        "{} {} file(s) from {}",
        style("⚑ commit plan").color256(theme::ACCENT).bold(),
        plan.paths.len(),
    ...
    let msg = if message.trim().is_empty() {
        ...
        if task.is_empty() {
            format!("fauto session {}", plan.session_id)
        } else {
            task.to_string()
        }
    } else {
...
/// `/work` — isolated worktree mode: one fauto window per linked worktree + branch.
    ...
    match sub {
        "" | "list" | "ls" => match coop::work_list() {
            Ok(list) if list.is_empty() => tui::emit_line(
                &style("no fauto worktrees — create one with /work new <name>")
                    .dim()
                    .to_string(),
            ),
            Ok(list) => {
                tui::emit_line(&format!(
                    "{}  {} worktree(s)",
                    style("⚑ work").color256(theme::ACCENT).bold(),
                    list.len()
                ));
                for w in &list {
    ...
    match builtin.id {
            ...
            if arg.trim().is_empty() {
                ...
                match cancellable_slash(handoff_now(history, arg.trim())).await {
                    Some(Ok(summary)) => {
                        ...
                        tui::emit_line(&style("handoff — fresh thread seeded with the relevant context").color256(theme::ACCENT).to_string());
        ...
        SlashId::Approval => {
            ...
            if requested.is_empty() || matches!(requested, "status" | "st") {
                ...
                match cli_config::save(&cfg) {
                    Ok(_) => tui::emit_line(&style(format!("approval → {mode}")).color256(theme::ACCENT).to_string()),
        ...
        SlashId::Sandbox => {
            ...
            if requested.is_empty() || matches!(requested, "status" | "st") {
                ...
                tui::emit_line(
                    &style(format!(
                        "sandbox: {mode} · backend {} · fs write {} · network deny {} · env {} — `fauto sandbox status` for the full matrix",
...
SlashId::Yolo => {
            ...
            tui::emit_line(&style(format!("approval → {mode} (legacy /yolo alias)")).color256(theme::ACCENT).to_string());
        }
        SlashId::AutoCopy => {
        ...
        SlashId::Smart => {
            ...
            cfg.set_approval_mode(mode);
            let _ = cli_config::save(&cfg);
            tui::emit_line(&style(format!("approval → {mode} (legacy /smart alias)")).color256(theme::ACCENT).to_string());
        }
        SlashId::Ultimate => {
            let mut cfg = cli_config::load();
            ...
            // `✦ ultimate` status chip), accent-dim when OFF. Reflects the effective state — an
        ...
        SlashId::Undo => match timemachine::undo() {
            Ok(s) => tui::emit_line(&format!("{} checkpoint #{}", style("⏪ rewound to").color256(theme::ACCENT), s.id)),
            Err(e) => tui::emit_line(&style(format!("undo: {e}")).color256(crate::ui::theme::WARN).to_string()),
        },
        SlashId::Redo => match timemachine::redo() {
            Ok(s) => tui::emit_line(&format!("{} checkpoint #{}", style("⏩ re-applied").color256(theme::ACCENT), s.id)),

...
//! they touched.
//!
//! F.Auto is routinely opened in several terminals at once against ONE working tree — auth in this
...
//! the turn ends (see `agent::execute_calls`). Two fauto sessions on one worktree are therefore
...
pub fn peers_banner() -> Option<String> {
    ...
    Some(format!(
        "⚑ {} other fauto session(s) in this repo — {} · /team status",
        peers.len(),
        bits.join(", ")
    ))
}
...
pub fn resolve(reference: &str) -> Result<SessionView> {
    ...
    if sessions.is_empty() {
        bail!("no fauto sessions are registered in this repository yet");
    }
...
/// set of interval boundaries. Manual checkpoints (`fauto time save`) are deliberately excluded: they
...
/// Every fauto-created worktree of this repository, with the state that decides whether removing it
/// would lose work.
pub fn work_list() -> Result<Vec<WorkTree>> {
    ...
    for entry in rd.flatten() {
        ...
        let branch = current_branch(&path).unwrap_or_else(|| format!("fauto/{name}"));
...
pub fn work_remove_blockers(wt: &WorkTree) -> Vec<String> {
    ...
    if wt.sessions > 0 {
        blockers.push(format!(
            "{} fauto session(s) still running in it",
            wt.sessions
        ));
    }
    blockers
...
pub fn work_remove(name: &str, force: bool) -> Result<String> {
    ...
    let wt = work_list()?
        .into_iter()
        .find(|w| w.name == name)
        .with_context(|| format!("no fauto worktree named '{name}'"))?;
...
impl crate::agent::tools::Tool for TeamStatus {
    ...
    fn description(&self) -> &str {
        "List the other fauto sessions working in this repository right now: their task, state \
    ...
    fn execute(&self, args: &serde_json::Value) -> Result<String> {
        ...
        let sessions = list();
        if sessions.is_empty() {
            return Ok("no fauto sessions are registered in this repository".to_string());
        }
        let mut out = String::new();
        for v in &sessions {
            let m = &v.manifest;

...
fn config_provider_subcommands_parse() {
    for argv in [
        ...
        vec!["fauto", "config", "provider", "use", "backup"],
        vec!["fauto", "config", "provider", "list"],
        vec!["fauto", "config", "provider", "remove", "backup", "--force"],
    ] {
        assert!(
            matches!(
    ...
    assert!(Cli::try_parse_from(["fauto", "config", "provider", "add", "missing-flags"]).is_err());
}
...
/// natural `fauto agent --save-session "fix the delete button"` would file an empty task under a
...
fn agent_save_session_is_a_switch_and_never_eats_the_task() {
    let cli = Cli::try_parse_from(["fauto", "agent", "--save-session", "sửa cái nút xoá"])
        .expect("parse");
    ...
    let plain = Cli::try_parse_from(["fauto", "agent", "chạy test"]).expect("parse");
...
fn agent_effort_takes_the_ladder_and_nothing_else() {
    ...
    for tier in ["auto", "low", "medium", "high", "xhigh", "max"] {
        assert_eq!(
            effort_of(&["fauto", "agent", "--effort", tier, "việc"]).as_deref(),
            Some(tier)
        );
    }
    ...
    assert_eq!(effort_of(&["fauto", "agent", "việc"]), None);

    assert!(Cli::try_parse_from(["fauto", "agent", "--effort", "mediumish", "việc"]).is_err());
}
...
fn agent_image_is_repeatable_and_never_eats_the_task() {
    ...
    assert!(Cli::try_parse_from(["fauto", "agent", "--image"]).is_err());
}
...
fn memory_list_takes_its_scope_positionally_or_as_a_flag() {
    ...
    assert_eq!(
        scope_of(&["fauto", "memory", "list", "current"]).as_deref(),
        Some("current"),
        "the positional form is what the REPL teaches"
    );
    assert_eq!(
        scope_of(&["fauto", "memory", "list", "--scope", "current"]).as_deref(),
        Some("current"),
        "the flag form shipped first and must keep working"
    );
    assert_eq!(
        scope_of(&["fauto", "memory", "list"]),
        None,
        "no scope ⇒ the unfiltered view"
    );
    assert!(
        Cli::try_parse_from(["fauto", "memory", "list", "current", "--scope", "global"]).is_err(),
...
fn where_reports_name_every_store_and_flag_missing_dirs() {
    ...
    let home = std::env::temp_dir().join(format!("fauto-whererep-{}", std::process::id()));
...
fn where_subcommands_parse() {
    assert!(matches!(
        Cli::try_parse_from(["fauto", "memory", "where"])
            .expect("parse")
            .command,
        Some(Commands::Memory {
            cmd: MemoryCmd::Where
    ...
    assert!(matches!(
        Cli::try_parse_from(["fauto", "skill", "where"])
            .expect("parse")
            .command,
        Some(Commands::Skill {
            cmd: SkillCmd::Where
...
fn a_session_with_images_survives_save_and_load() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-img-session-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
...
fn most_recent_session_prefers_this_project_over_a_newer_foreign_one() {
    ...
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-recent-scope-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
    set_session_slug(None);
...
fn session_files_carry_provenance_and_legacy_arrays_still_load() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-sess-prov-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
    set_session_slug(None);
    let dir = sessions_dir();
...
fn load_session_rebuilds_stale_lanes_for_the_current_project() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-sess-lanes-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
    set_session_slug(None);
    let dir = sessions_dir();
...
fn scan_sessions_classifies_mine_foreign_unlabeled_and_corrupt() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-scan-classes-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
    set_session_slug(None);
    let dir = sessions_dir();
...
/// `fauto zone migrate` must re-home SESSIONS too. They are the one artifact keyed by provenance
...
fn zone_migrate_rehomes_sessions_carrying_a_legacy_slug() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-zone-sess-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
    set_session_slug(None);
    let dir = sessions_dir();
...
fn restoring_the_legacy_last_pointer_rehomes_it_to_a_named_file() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-last-rehome-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
    set_session_slug(None);
    let dir = sessions_dir();
...
fn handoff_seed_survives_a_lane_rewrite() {
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-handoff-seed-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
...
fn classify_health_probe_rules() {
    ...
    let missing = classify_health_probe(Err(anyhow!("no API key — run `fauto config`")));
    assert_eq!(
        missing,
        tui::HealthKind::Unstable,
...
fn allocate_session_slug_never_reuses_an_existing_file() {
    ...
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-slug-alloc-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);
...
fn exit_flush_persists_the_live_conversation_for_sessions() {
    ...
    let _g = crate::core::config::TEST_HOME_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let home = std::env::temp_dir().join(format!("fauto-exit-flush-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&home);
    std::env::set_var("AIZEN_HOME", &home);

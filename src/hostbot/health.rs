//! Liveness for `fauto serve` — a heartbeat file the daemon touches and `fauto serve --health`
//! reads.
//!
...
/// `fauto serve --health`: print one line and return whether the daemon is healthy. The caller turns
...
pub fn run_health_check() -> bool {
    let (line, ok) = match read() {
        None => (
            format!(
                "fauto serve: no heartbeat at {} — daemon not running",
        ...
        Some(hb) => {
            ...
            match hb.verdict(now_secs(), max_idle_secs(), max_busy_secs()) {
                Ok(msg) => (
                    format!("fauto serve [{platform}]{where_} healthy — {msg}{lanes}"),
                    true,
                ),
                Err(msg) => (
                    format!("fauto serve [{platform}]{where_} UNHEALTHY — {msg}{lanes}"),
                    false,
                ),
            }
        }
...
mod tests {
    ...
    fn beat_round_trips_and_carries_since_across_a_same_state_restamp() {
        ...
        let tmp = std::env::temp_dir().join(format!("fauto-hb-test-{}", std::process::id()));
    ...
    fn two_platforms_on_one_machine_do_not_overwrite_each_other() {
        ...
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let tmp = std::env::temp_dir().join(format!("fauto-hb-split-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
        std::env::set_var("AIZEN_HOME", &tmp);
    ...
    fn a_legacy_shared_heartbeat_is_still_readable_after_upgrade() {
        ...
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let tmp = std::env::temp_dir().join(format!("fauto-hb-legacy-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
        std::env::set_var("AIZEN_HOME", &tmp);
    ...
    fn beat_lanes_records_the_lane_counts_and_the_machine() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let tmp = std::env::temp_dir().join(format!("fauto-hb-lanes-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
        std::env::set_var("AIZEN_HOME", &tmp);

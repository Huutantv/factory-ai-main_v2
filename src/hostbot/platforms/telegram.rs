...
//! auditable token handling). Powers the `fauto serve` daemon (via `TelegramPlatform`), the
...
//!
//! Approval routing: when running under `fauto serve` (a single poll loop owns `getUpdates`), an
//! approval request registers a `oneshot` in a global PENDING map; the poll loop delivers the
//! callback → resolves it. Standalone (`fauto agent`, no daemon) there is no other poller, so the
...
impl Client {
    ...
    /// `getMe` — used by `fauto telegram test` to validate the token.
...
/// True while `fauto serve` owns the single poll loop (so approvals route through PENDING, not a
...
///   3. the primary `configured()` bot — standalone `fauto agent`, no daemon.
...
async fn self_poll_for_approval(client: &Client, id: &str) -> bool {
    ...
    while start.elapsed() < Duration::from_secs(APPROVAL_TIMEOUT_SECS) {
        let updates = match client.get_updates(offset, 20).await {
            Ok(u) => u,
            Err(e) => {
                // 409 Conflict = another process (almost always a running `fauto serve` daemon) already
                ...
                if !warned_conflict && e.to_string().contains("409") {
                    ...
                    eprintln!(
                        "[telegram] getUpdates 409 Conflict — another process (likely `fauto serve`) is \
...
impl Tool for TelegramSend {
    ...
    fn execute(&self, args: &Value) -> Result<String> {
        ...
        block(async {
            let (client, cfg) =
                configured().context("telegram not configured (run `fauto telegram setup`)")?;
...
impl Tool for TelegramAsk {
    ...
    fn execute(&self, args: &Value) -> Result<String> {
        ...
        match block(async { anyhow::Ok(request_approval(q).await) })? {
            ...
            None => Ok("error: telegram not configured (run `fauto telegram setup`)".to_string()),
        }
    }
}
...
/// `fauto serve` to get a fresh code). The bot controls the machine, so pairing is deliberately
...
impl TelegramPlatform {
    ...
    pub fn from_config_selecting(menu: Vec<(String, String)>, wanted: Vec<String>) -> Result<Self> {
        ...
        let token = cfg.resolved_token().context(
            "no telegram bot token — run `fauto telegram setup` or `fauto serve --token <token>`",
        )?;
        Ok(Self {
...
async fn spawn_bot(
    ...
    .map_err(|_| {
        anyhow::anyhow!(
            "another F.Auto process on this machine is already polling this bot token — \
                    ...
                    if let Some(code) = pairing.clone() {
                        if tokio::time::Instant::now() >= deadline {
                            ...
                            let _ = client
                                .send_message(
                                    chat,
                                    "Pairing window closed — restart `fauto serve` to pair.",
                                )
                                .await;
...
async fn do_add_bot(
    ...
    if name == "default" {
        bail!(
            "\"default\" is the primary bot — manage it on the host with `fauto telegram setup`."
        );
    }
    {
...
async fn do_remove_bot(bots: &Arc<Mutex<HashMap<String, BotHandle>>>, name: &str) -> Result<()> {
    if name == "default" {
        bail!("cannot remove the primary bot — use `fauto telegram disable` on the host.");
    }
...
fn do_set_persona(
    ...
    if name == "default" {
        bail!("\"default\" uses the primary agent's own persona — set it on the host with `fauto persona`.");
    }
...
/// common path is the REPL with no daemon running: persist, then `fauto serve` boots it (pairing).
async fn admin_set_primary(token: &str) -> Result<String> {
    ...
    Ok(match bot_control() {
        Some(_) => format!(
            "primary bot token saved (@{username}) — restart `fauto serve` to switch the primary bot over."
        ),
        None => format!(
            "primary bot set to @{username} — run `fauto serve` (it prints a pairing code if there's no owner yet)."
        ),
    })
}
...
async fn admin_add_bot(name: &str, token: &str, own_owner: bool) -> Result<String> {
    ...
    match bot_control() {
        ...
        None => {
            ...
            add_bot_entry(name, token, own_owner)?;
            Ok(format!(
                "saved @{user} as \"{name}\" — it will start on the next `fauto serve`"
            ))
        }
    }
}
...
impl Tool for BotAdmin {
    ...
    fn description(&self) -> &str {
        "Manage the Telegram bots this daemon hosts, on the owner's request. Actions: \
         ...
         next `fauto serve`); `remove` (stop hosting a bot — cannot remove \"default\"); `set_persona` \
...
mod tests {
    ...
    fn with_temp_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-tg-{tag}-{}", std::process::id()));

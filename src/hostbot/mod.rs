//! `hostbot` — the self-contained "host fauto as a chat bot" feature.
//!
//! One process hosts ONE chat platform (`fauto serve` = Telegram, `fauto discord serve` = Discord),
...
//! auth, set up with `fauto telegram/discord setup`). Self-host wiring (systemd) is in [`service`].

...
//! auto-start on reboot). `fauto serve --install` writes the unit; `--uninstall` removes it. On
...
/// Render the systemd unit that runs `fauto serve` as an always-on service. `exec` is the absolute
...
fn systemd_unit_text(exec: &std::path::Path, workdir: &std::path::Path, user: bool) -> String {
    ...
    format!(
        "[Unit]\n\
         Description=fauto — Telegram control daemon\n\
...
/// `fauto serve --install` / `--uninstall`: wire (or remove) the systemd service. On non-Linux we
...
pub async fn run_serve_service(
    ...
    let exec = std::env::current_exe().context("finding the fauto binary path")?;
    ...
    if uninstall {
        if user {
            let path = home.join(".config/systemd/user/fauto-serve.service");
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "fauto-serve"])
                .status();
            ...
            let path = std::path::PathBuf::from("/etc/systemd/system/fauto-serve.service");
            let _ = std::process::Command::new("systemctl")
                .args(["disable", "--now", "fauto-serve"])
                .status();
            let _ = std::fs::remove_file(&path);
            let _ = std::process::Command::new("systemctl")
                .arg("daemon-reload")
            ...
            println!(
                "{}",
                style("✓ removed system service").color256(theme::ACCENT)
            );
        } else {
            println!("Run these as root to remove the system service:");
            println!("  sudo systemctl disable --now fauto-serve");
            println!("  sudo rm /etc/systemd/system/fauto-serve.service");
    ...
    if install {
        if user {
            ...
            let path = dir.join("fauto-serve.service");
            std::fs::write(&path, systemd_unit_text(&exec, &home, true))
                .with_context(|| format!("writing {}", path.display()))?;
            println!(
                "{}",
                style(format!("✓ wrote {}", path.display())).color256(theme::ACCENT)
            );
            if now {
                let _ = std::process::Command::new("systemctl")
                    .args(["--user", "daemon-reload"])
                ...
                let st = std::process::Command::new("systemctl")
                    .args(["--user", "enable", "--now", "fauto-serve"])
                    .status();
                ...
                let _ = std::process::Command::new("loginctl")
                    ...
                    .status();
                match st {
                    Ok(s) if s.success() => println!("{}", style("✓ enabled + started (survives logout + reboot via linger)").color256(theme::ACCENT)),
                ...
                println!("  systemctl --user enable --now fauto-serve");
            ...
            if now {
                let _ = std::process::Command::new("systemctl")
                    ...
                    .status();
                let _ = std::process::Command::new("systemctl")
                    .args(["enable", "--now", "fauto-serve"])
                    .status();
                println!(
                    "{}",
                    style("✓ enabled + started (auto-starts on reboot)").color256(theme::ACCENT)
                );
            } else {
                println!("Then: systemctl daemon-reload && systemctl enable --now fauto-serve");
            }
        } else {
            // System install needs root — print the unit + the sudo steps rather than silently failing.
            let path = "/etc/systemd/system/fauto-serve.service";
            println!(
                "System install needs root. Either re-run with sudo, or use --user (no root):"
            );
            println!("  fauto serve --install --user --now");
            println!("\nOr write this unit as root at {path}:\n");
            println!("{}", systemd_unit_text(&exec, &home, false));
            println!(
                "Then: sudo systemctl daemon-reload && sudo systemctl enable --now fauto-serve"
            );
        }
        return Ok(());
    }
...
pub async fn run_serve_service(
    ...
    println!("`fauto serve --install` wires a systemd service — Linux only.");
    if cfg!(target_os = "windows") {
        println!("On Windows, run the daemon as a service with NSSM (https://nssm.cc):");
        println!("  nssm install fauto \"<path>\\fauto.exe\" serve");
        println!("or as a Scheduled Task set to run at logon with 'Restart on failure'.");
    } else {
        println!("On macOS, wrap `fauto serve` in a launchd plist under ~/Library/LaunchAgents.");
    }
    Ok(())
}
...
mod tests {
    ...
    fn systemd_unit_has_restart_and_exec() {
        let exec = std::path::Path::new("/home/vps/.cargo/bin/fauto");
        ...
        assert!(
            user_unit.contains("ExecStart=/home/vps/.cargo/bin/fauto serve"),
            "runs `serve`"
        );
        assert!(

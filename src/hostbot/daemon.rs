...
//!
//! `fauto serve` (Telegram) and `fauto discord serve` (Discord) both funnel into `run_daemon<P>`:
...
fn serve_help_text() -> String {
    let mut s = String::from("F.Auto bot — control this machine from chat. Commands:\n");
...
async fn run_daemon<P: Platform>(platform: P) -> Result<()> {
    ...
    let (base_url, api_key, model) = crate::resolve_endpoint(None, None, None)
        .context("configure the model endpoint first (run `fauto config`)")?;
    ...
    eprintln!(
        "{}",
        style(format!(
            "fauto serve — {} live, up to {} turn(s) at once (Ctrl-C to stop)",
            platform.name(),
            lane::max_concurrent()
        ))
        .dim()
    ...
    // Liveness for container/orchestrator probes (`fauto serve --health`). Stamped from INSIDE this
...
/// `fauto serve` — host the Telegram platform (primary bot + any `/addbot`-hosted extras).
...
/// `fauto discord serve` — host the Discord platform (one gateway, full command menu, no inline
...
mod tests {
    ...
    fn with_temp_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        let _g = crate::core::config::TEST_HOME_LOCK
            ...
            .unwrap_or_else(|e| e.into_inner());
        let dir =
            std::env::temp_dir().join(format!("fauto-daemon-lane-{tag}-{}", std::process::id()));
    ...
    fn a_deleted_lane_directory_falls_back_instead_of_failing_every_turn() {
        ...
        with_temp_home("cwd-gone", || {
            let gone = std::env::temp_dir().join("fauto-lane-gone-does-not-exist");
    ...
    async fn the_lane_model_is_what_lands_in_the_chat_completions_body() {
        ...
        let model = {
            let _g = crate::core::config::TEST_HOME_LOCK
                .lock()
                .unwrap_or_else(|e| e.into_inner());
            let dir = std::env::temp_dir().join(format!("fauto-wire-{}", std::process::id()));
            let _ = std::fs::remove_dir_all(&dir);
            std::fs::create_dir_all(&dir).unwrap();
            std::env::set_var("AIZEN_HOME", &dir);
            store::update_lane("bot-a", |l| {

//! `fauto` — F.Auto first-party agentic coding CLI.
//!
//! Subcommands:
//!   fauto chat    — OpenAI-compatible streaming chat (the v0 "call API like hermes" layer)
//!   fauto memory  — the standalone, best-for-CLI memory brain (see linked-riding-mochi.md)
...
mod cli; // one module per `fauto <subcommand>` — the behaviour behind `cli_args`
...
/// A one-accent (magenta, matching the palette) theme — dim for secondary, bold for the active row.
...
/// The interactive surface (bare `fauto`). Dispatches to the **sticky TUI** (pinned bottom input box +
...
async fn run_menu() -> Result<()> {
    ...
    if !forced && !std::io::stdin().is_terminal() {
        println!("fauto — F.Auto agentic CLI");
        println!(
            "Run `fauto --help` for commands, or `fauto config` to set up the endpoint + key."
        );
        return Ok(());
    }
...
pub(crate) fn status_text(history: &[Message], model: &str) -> String {
    ...
    // Active persona chip — so it's always visible WHICH character fauto is role-playing (not just a
...
async fn run_menu_sticky() -> Result<()> {
    ...
    tui::set_ultimate(cli_config::ultimate_enabled()); // open the input box in the right colour (warn-yellow if ultimate)
    ...
    sweep_orphan_temps();
    // Publish this window to the repository's session registry so the OTHER fauto windows (and the
        ...
        match sub {
            ...
            tui::Submission::Chat(line, images) => {
                ...
                tui::emit_line(&format!(
                    "{} {}",
                    style("❯").color256(theme::ACCENT).bold(),
                    style(&echo).color256(theme::ACCENT)
                ));
                ...
                match result {
                    ...
                    Some(Ok(outcome)) => {
                        ...
                        // tree is remembered as a finished task. The one-shot `fauto agent` path has

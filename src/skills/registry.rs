...
mod tests {
    ...
    fn a_slug_is_one_encoded_segment() {
        ...
        assert_eq!(
            by_slug_url("https://reg.example.com/fauto/", "o/n")
                .unwrap()
                .as_str(),
            "https://reg.example.com/fauto/api/agent/skills/o%2Fn/install"
        );
    ...
    fn search_refuses_a_private_registry_base() {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-registry-ssrf-{}", std::process::id()));

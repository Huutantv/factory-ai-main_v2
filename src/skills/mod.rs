...
/// `~/.aizen/skills/` — the personal GLOBAL skill dir; `skill_save`/`fauto skill add` write here.
...
/// surface hasn't been published (tests / offline `fauto skill`), don't filter (`None` → show).
...
/// (`fauto skill list --all-zones`); never enters the index or `load()`.
...
mod tests {
    ...
    fn with_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-skill-{tag}-{}", std::process::id()));
    ...
    fn project_zone_skill_visible_only_in_its_workspace() {
        with_home("zone", || {
            ...
            let other =
                std::env::temp_dir().join(format!("fauto-skill-otherzone-{}", std::process::id()));

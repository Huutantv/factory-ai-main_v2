//! `fauto zone …`, `fauto team …` and `fauto work …` — the non-interactive twins of `/zone`,
//! `/team` and `/work`.
//!
...
pub(crate) fn run_zone(cmd: ZoneCmd) -> Result<()> {
    match cmd {
        ZoneCmd::Migrate { apply } => {
            ...
            if !apply {
                println!("\ndry-run — nothing was changed. Re-run with `fauto zone migrate --apply` to merge into {}.", plan.current_slug);
                return Ok(());
            }
...
/// `fauto team …` — the non-interactive twin of `/team`. Same registry, same plan, `println!` instead
...
pub(crate) fn run_team(cmd: TeamCmd) -> Result<()> {
    match cmd {
        ...
        TeamCmd::Commit {
            ...
            // `--yes` is the only path to an actual commit: a bare `fauto team commit` reviews and
...
/// `fauto work …` — isolated worktrees, for when two windows should not share one tree at all.
pub(crate) fn run_work(cmd: WorkCmd) -> Result<()> {
    match cmd {
        WorkCmd::New { name } => {
            let wt = coop::work_new(&name)?;
            println!(
                "{} {}\n  branch {}\n  open a session there:  cd {} && fauto",
                style("✓ worktree").color256(theme::ACCENT),
                wt.path.display(),
                wt.branch,
                wt.path.display()
            );
        ...
        WorkCmd::Remove { name, force } => {
            ...
            println!("{} {msg}", style("✓").color256(theme::ACCENT));
            Ok(())
        }
    }
}

    ...
    match outcome.stop {
        StopReason::Done => {
            eprintln!(
                "{}",
                style(
                    "✓ Plan generation complete. Use `fauto agent` without --plan-only to execute."
                )
                .dim()
            );
        }
...
/// `fauto reach doctor [--json]` / `fauto reach status` — the web-access health check.
...
/// Run the agent loop once (non-streaming, quiet) and return its final text — used by `fauto serve`
...
pub(crate) async fn run_agent_capture(
    ...
    // A `clarify` yield in a captured (non-REPL) run — e.g. `fauto serve` — has no input box to loop
...
pub(crate) async fn run_models(args: ModelsArgs) -> Result<()> {
    ...
    println!("\nset a default: `fauto config set --model <id>`");
    Ok(())
}
...
pub(crate) async fn run_agent_cmd(args: AgentArgs) -> Result<()> {
    ...
    match outcome.stop {
        ...
        // One-shot `fauto agent` is non-interactive: there is no next message to answer with, so
        // surface the question and exit rather than hang. Re-run in the REPL to answer it.
        StopReason::AwaitingInput(q) => eprintln!(
            "\n[the agent needs clarification — re-run interactively (`fauto`) to answer]\n❓ {q}"
        ),
...
mod tests {
    ...
    fn a_saved_one_shot_lands_in_the_pool_with_its_provenance() {
        ...
        let home = std::env::temp_dir().join(format!("fauto-oneshot-save-{}", std::process::id()));
    ...
    fn a_run_with_no_user_turn_writes_nothing() {
        let _g = crate::core::config::TEST_HOME_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let home = std::env::temp_dir().join(format!("fauto-oneshot-empty-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::env::set_var("AIZEN_HOME", &home);
        set_session_slug(None);
        std::fs::create_dir_all(sessions_dir()).unwrap();

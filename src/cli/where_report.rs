//! `fauto where` / `/where` — the identity report: which project this is, which slug it hashed to,
    ...
    let _ = write!(
        ...
        style(
            "\nEdit or delete files directly — they are plain markdown with a frontmatter header.\n\
             Re-run `fauto memory doctor` afterwards to catch anything left dangling."
        )
        .dim()
    );
    s
...
/// Shared verbatim by `fauto where` (println) and `/where` (tui::emit_line).
pub(crate) fn where_report() -> String {
    ...
    if let Some(l) = crate::features::zones::quick_legacy_probe() {
        let _ = writeln!(
            s,
            "⚠ legacy zone : {l} — data from the old slug keying; `fauto zone migrate` shows what would merge (--apply to do it)"
        );
    }

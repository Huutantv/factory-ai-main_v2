//! `fauto time …` and `/timemachine` — the git-backed checkpoint timeline.
//!
...
pub(crate) fn run_time(cmd: TimeCmd) -> Result<()> {
    match cmd {
        ...
        TimeCmd::Restore { id } => {
            ...
            println!(
                "{} #{} — {label}",
                style("⏪ restored to").color256(theme::ACCENT),
                snap.id
            );
        ...
        TimeCmd::Undo => {
            ...
            println!(
                "{} #{}",
                style("⏪ undo →").color256(theme::ACCENT),
                snap.id
            );
            Ok(())
        }
        TimeCmd::Redo => {
            ...
            println!(
                "{} #{}",
                style("⏩ redo →").color256(theme::ACCENT),
                snap.id
            );
            Ok(())
        }
        TimeCmd::Prune { keep } => {
            ...
            println!(
                ...
                style("🧹 pruned").color256(theme::ACCENT)
            );
            Ok(())
        }
        TimeCmd::Doctor { json, repair } => {
            ...
            if json {
                ...
                if report.loose_objects >= 1024 {
                    println!(
                        "  {} loose object(s) not packed — run `fauto time gc` to compact",
            ...
            if all {
                ...
                if report.orphans.is_empty() {
                    println!(
                        ...
                        style("🧹 time gc --all:").color256(theme::ACCENT),
                        report.stores.len()
                    );
                } else {
                ...
                println!(
                    "{} repo {} · worktree {} · {} checkpoint(s)",
                    style("🧹 time metadata cleaned:").color256(theme::ACCENT),
                    report.repo_id,
                    report.worktree_id,
                    report.checkpoints
                );
        ...
        TimeCmd::Clear => {
            ...
            println!(
                ...
                style("🧹 cleared").color256(theme::ACCENT)
            );
            Ok(())
        }
    }
...
fn resolve_diff_sides(
    ...
    let parse = |s: &str| {
        DiffSide::parse(s).with_context(|| {
            format!("`{s}` is not a checkpoint id or `working` (try `fauto time list`)")
        })
    };
    match (from, to) {
        (None, _) => {
            ...
            let cur = cursor.and_then(|i| snaps.get(i)).map(|s| s.id).context(
                "no checkpoints yet — nothing to diff against (`fauto time save` first)",
            )?;
...
/// Render a diff report as display lines. Shared so `fauto time diff` (stdout) and `/diff` (the TUI,
...
pub(crate) fn diff_lines(report: &timemachine::DiffReport, narrow_hint: &str) -> Vec<String> {
    ...
    let mut out = vec![format!(
        "{}  {} → {}  ·  {} file(s), {}",
        style("⎇ diff").color256(theme::ACCENT).bold(),
        report.from,
        report.to,
        report.files.len(),
        style(format!(
...
/// `fauto time diff` — print the changes between two points in the timeline.
fn run_time_diff(
    from: Option<String>,
    to: Option<String>,
    paths: Vec<String>,
...
/// `fauto time list` — a static, glanceable print of the checkpoint timeline (newest first), with the
...
fn print_timeline() -> Result<()> {
    ...
    tui::emit_line(&format!(
        "{}  {n} checkpoint(s)",
        style("⎇ timeline").color256(theme::ACCENT).bold(),
    ));
    ...
    for (i, s) in snaps.iter().enumerate().rev() {
        ...
        let body = if is_cur {
            format!(
                "{} {}",
                style(mark).color256(theme::ACCENT).bold(),
                style(head).color256(theme::ACCENT)
            )
        } else {
            format!("{mark} {head}")
        };
    ...
    tui::emit_line(
        &style("▸ = current · restore: fauto time restore <id>   (or /timemachine in the REPL)")
            .dim()
            .to_string(),
    );
    Ok(())
...
fn restore_checkpoint(
    ...
    *history = chat;
    ...
    println!(
        ...
        style("⏪ restored").color256(theme::ACCENT),
        snap.id
    );
    println!(
        "{}",

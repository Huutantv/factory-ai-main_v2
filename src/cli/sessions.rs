//! `/sessions`, `/import` and `fauto import` — loading a saved conversation over the live one.
//!
            ...
            if !name.trim().is_empty() {
                ...
                match save_session(history, name.trim(), Some(model_label)) {
                    Ok(_) => {
                        ...
                        println!(
                            "{}",
                            style(format!("saved '{}'", name.trim())).color256(theme::ACCENT)
                        );
                    }
                ...
                .interact_opt()
            {
                ...
                match delete_session(slug) {
                    Ok(_) => {
                        ...
                        println!(
                            "{}",
                            style(format!("deleted '{pretty}'")).color256(theme::ACCENT)
                        );
                    }
...
/// it inside fauto. The foreign transcript is parsed into `Vec<Message>`, repaired so it satisfies
...
/// the live one and is autosaved under a fresh fauto slug from then on, exactly like a `/resume`
/// of an fauto session. The source file is only ever READ.
pub(crate) async fn import_menu(history: &mut Vec<Message>, model_label: &str) -> Result<()> {
    ...
    match features::foreign_session::load(session) {
        Ok(imported) => {
            ...
            // but defend against a future schema that embeds them) before splicing fauto's own.
            *history = imported;
            ...
            set_session_slug(None); // the imported chat autosaves under a fresh fauto slug from here
            agent::replay_transcript(history);
...
/// `fauto import [path]` — CLI surface. No path lists every foreign transcript for this project;

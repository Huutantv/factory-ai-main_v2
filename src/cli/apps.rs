//! `fauto apps …` — connecting apps through the MCP registry.
//!
...
/// `fauto apps …` — connect apps via the MCP registry.
pub(crate) async fn run_apps(cmd: Option<AppsCmd>) -> Result<()> {
    match cmd {
        ...
        Some(AppsCmd::Search { query, limit }) => {
            let q = query.join(" ");
            if q.trim().is_empty() {
                return Err(anyhow!("usage: fauto apps search <keywords>"));
            }
            ...
            println!(
                "{}",
                style(format!(
                    "{} result(s) from {} — `fauto apps add <name>` to connect:",
                    hits.len(),
            ...
            for s in &hits {
                let name = style(&s.name).color256(theme::ACCENT);
                println!("  {name}\n    {}", s.summary_line());
            }
            Ok(())
        }
        ...
        Some(AppsCmd::Remove { name }) => {
            if app_catalog::remove_server(&name)? {
                ...
                println!(
                    "{}",
                    style(format!("✓ disconnected '{name}'.")).color256(theme::ACCENT)
                );
            } else {
                println!("no connected app keyed '{name}' (see `fauto apps list`).");
            }
            Ok(())
        }
    }
...
fn apps_print_list() {
    ...
    println!(
        "{}",
        style("Apps — connect via the MCP registry (`fauto apps add <key>`):").bold()
    );
...
pub(crate) async fn apps_add(name: &str) -> Result<()> {
    ...
    if viable.is_empty() {
        return Err(anyhow!(
            "no connectable '{label}' server found on the registry (only legacy sse-only entries, which fauto's client doesn't speak). Run `fauto apps search {query}` to explore."
        ));
    }
    ...
    let choice = app_catalog::pick_transport_for_install(&server)
        .context("this server declares no transport fauto can use")?;
    let repo = server
        .repository
        .as_ref()
        .map(|r| r.url.clone())
    ...
    if let app_catalog::TransportChoice::OAuthRemote(i) = choice {
        ...
        println!(
            "  {}",
            style(format!(
                "🔐 sign-in app @ {host} — F.Auto will open your browser to authorize."
            ))
            .dim()
        );
        let go = Confirm::with_theme(&theme)
    ...
    // the entry is written, the user can retry with `fauto apps login <key>`.
...
/// `fauto apps info <key>` — the detail view for ONE connected app: its mcp.json config (transport +
...
pub(crate) async fn apps_info(key: &str) -> Result<()> {
    let Some(entry) = app_catalog::installed_entry(key) else {
        return Err(anyhow!(
            "no connected app keyed '{key}' — see `fauto apps list`"
        ));
    };
    println!("{}", style(key).color256(theme::ACCENT).bold());
    print_entry_summary(&entry, Some(key));

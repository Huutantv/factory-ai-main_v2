//! `fauto agents …` — the specialist sub-agent library (agency-agents format).
//!
...
pub(crate) async fn run_agents(cmd: Option<AgentsCmd>) -> Result<()> {
    match cmd {
        ...
        Some(AgentsCmd::Show { name }) => match agents::load(&name) {
            ...
            None => anyhow::bail!("no agent named '{name}' (try `fauto agents list`)"),
        },
        ...
        Some(AgentsCmd::Remove { name }) => {
            if agents::delete_home(&name)? {
                ...
                println!("(no agent named '{name}' under ~/.aizen/agents — `fauto agents where` shows the dirs)");
            }
            Ok(())
        }
        ...
        Some(AgentsCmd::SetProvider {
            ...
            if agents::load(&name).is_none() {
                anyhow::bail!("no agent named '{name}' (try `fauto agents list`)");
            }
            if clear {
...
/// Bare `fauto agents`: list when any exist, else the install nudge.
...
fn agents_nudge() {
    ...
    println!(
        "  {}",
        style("fauto agents install msitarzewski/agency-agents").color256(theme::ACCENT)
    );
...
fn agents_set_enabled(name: Option<&str>, all: bool, on: bool) -> Result<()> {
    ...
    let def = agents::load(name)
        .with_context(|| format!("no agent named '{name}' (try `fauto agents list`)"))?;

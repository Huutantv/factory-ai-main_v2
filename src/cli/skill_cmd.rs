//! `fauto skill …` — the skill store: list, show, save, refine, fetch, remove.
//!
    ...
    match cmd {
        SkillCmd::List { all_zones } => {
            ...
            if skills.is_empty() && !all_zones {
                println!(
                    "(no skills — add one with `fauto skill add <name>`, or /skills in the REPL)"
                );
                return Ok(());
            }
            ...
            if !retired.is_empty() {
                ...
                println!(
                    "\nretired: {} — `fauto skill restore <name>` brings one back",
                    names.join(", ")
                );
            }
            Ok(())
        ...
        SkillCmd::Show { name } => match skill::load(&name) {
            ...
            None => anyhow::bail!("no skill named '{name}' (try `fauto skill list`)"),
        },
        SkillCmd::Where => {
        ...
        SkillCmd::Delete { name } => {
            if skill::delete(&name)? {
                println!(
                    "retired '{name}' (archived — `fauto skill restore {name}` brings it back)"
                );
            } else {
        ...
        SkillCmd::Refine {
            ...
            println!(
                ...
                style("refined").color256(theme::ACCENT),
                archived.display()
            );
            Ok(())
        }
        ...
        SkillCmd::Search { query, limit } => {
            ...
            if q.trim().is_empty() {
                anyhow::bail!(
                    "a search query is required, e.g. `fauto skill search deploy fastapi`"
                );
            }
            ...
            println!(
                "{}",
                style(format!(
                    "{} result(s) from {} — install with `fauto skill install <owner/name>`",
                    hits.len(),
                    skill_registry::registry_base()
                ))
                .dim()

//! `fauto persona …` and `fauto soul …` — the operator's card and the durable operating identity.
//!
...
pub(crate) fn run_persona(cmd: PersonaCmd) -> Result<()> {
    match cmd {
        PersonaCmd::List => {
            ...
            let ps = persona::list();
            if ps.is_empty() {
                println!("(no personas yet — `fauto persona new <name>`, or /persona in the REPL)");
                return Ok(());
            }
            for p in &ps {
            ...
            if !retired.is_empty() {
                ...
                println!(
                    "\nretired: {} — `fauto persona restore <name>` brings one back",
                    names.join(", ")
                );
            }
        ...
        PersonaCmd::Use { name } => {
            let p = persona::load(&name)
                .ok_or_else(|| anyhow::anyhow!("no persona '{name}' (see `fauto persona list`)"))?;
        ...
        PersonaCmd::SelfMem { name, all } => {
            let slug = match name {
                ...
                None => persona::active_slug().ok_or_else(|| {
                    anyhow::anyhow!("no active persona — pass a name or `fauto persona use <name>`")
                })?,
            };
        ...
        PersonaCmd::Forget { id, name } => {
            let slug = match name {
                Some(n) => skill::sanitize_name(&n),
                None => persona::active_slug().ok_or_else(|| {
                    anyhow::anyhow!("no active persona — pass --name or `fauto persona use <name>`")
                })?,
            };
            persona::self_mem::forget(&slug, &id)?;
            println!(
                "retired self-memory '{id}' (archived — `fauto persona unforget {id}` restores it)"
            );
            Ok(())
        }
        ...
        PersonaCmd::Remember { text, importance } => {
            let slug = persona::active_slug().ok_or_else(|| {
                anyhow::anyhow!("no active persona — `fauto persona use <name>` first")
            })?;
        ...
        PersonaCmd::Delete { name } => {
            if persona::delete(&name)? {
                ...
                println!(
                    "card + self-memory archived under {} — `fauto persona restore {name}`",
        ...
        PersonaCmd::Block => {
            match persona::prompt_block() {
                ...
                None => {
                    println!("(no persona active — `fauto persona use <name>`)");
                    return Ok(());
                }
            }
            match persona::self_block() {
                Some(s) => println!("\n<self>\n{}\n</self>", s.trim()),
                None => println!("\n(no <self> yet — the character has no self-memory; `fauto persona remember \"...\"`)"),
            }
            Ok(())
        }
    }
    ...
    match cmd.unwrap_or(SoulCmd::Show) {
        SoulCmd::Show => {
            match soul::prompt_block() {
                ...
                None => println!(
                    "(no operating identity yet — set one with `fauto soul set` or edit {})",
                    soul::soul_path().display()
                ),
            }
            Ok(())

//! `fauto memory …` — the CLI half of the memory store (the REPL half is `/memory`).
//!
...
/// `fauto memory reconcile [--apply]` — the M2b batch pass, run by hand.
///
...
/// `fauto memory model-list` — show every model2vec model this machine already has, and which one
...
fn run_memory_model_list() -> Result<()> {
    ...
    if found.is_empty() {
        ...
        println!();
        println!("get one with: fauto memory model-download");
        return Ok(());
    }
    println!(

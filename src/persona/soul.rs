...
/// The raw SOUL.md text (for `fauto soul show`/editing), or `None` if absent/empty.
...
pub fn write(body: &str) -> Result<PathBuf> {
    if body.trim().is_empty() {
        anyhow::bail!("the SOUL body is empty (use `fauto soul clear` to remove it)");
    }
    let dir = aizen_home();
...
mod tests {
    ...
    fn with_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-soul-{tag}-{}", std::process::id()));

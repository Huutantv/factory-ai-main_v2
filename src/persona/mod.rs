...
/// `~/.aizen/personas/` — the personal (HOME) persona dir; `fauto persona new` writes here.
...
/// trusted (`fauto mcp trust` — the same supply-chain gate as project mcp.json/verify.json), and
...
mod tests {
    ...
    fn with_home<T>(tag: &str, f: impl FnOnce() -> T) -> T {
        ...
        let dir = std::env::temp_dir().join(format!("fauto-persona-{tag}-{}", std::process::id()));

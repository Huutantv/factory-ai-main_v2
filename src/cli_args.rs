...
pub(crate) struct Cli {
    ...
    /// (Also settable via AIZEN_SANDBOX or the `sandbox.mode` config key; see `fauto sandbox`.)
...
pub(crate) enum Commands {
    ...
    /// Other fauto windows working in this repository: status · diff · claims · commit.
    Team {
        #[command(subcommand)]
        cmd: TeamCmd,
    },
    ...
    /// Show where fauto keeps THIS project's state: root, zone slug, git executable, home dirs.
    Where,
...
pub(crate) enum AppsCmd {
    ...
    Info {
        /// The key shown by `fauto apps list`.
        name: String,
    },
...
pub(crate) enum AgentsCmd {
    ...
    List {
        ...
        /// Only this source: fauto-home | claude-home | fauto-project | claude-project.
        #[arg(short, long)]
    ...
    Show {
        /// Agent name or slug (from `fauto agents list`).
        name: String,
    },
...
pub(crate) enum McpCmd {
    ...
    /// fauto's specialists. Serves the repo it is started in.
    Serve {
...
pub(crate) enum SkillCmd {
    ...
    Install {
        /// The skill id, e.g. `NousResearch/spike` (from `fauto skill search`).
        slug: String,
    },
}
...
pub(crate) enum TimeCmd {
    ...
    Restore {
        /// Checkpoint id (from `fauto time list`).
        id: u32,
    },
    ...
    /// Delete ALL checkpoints (run `fauto time gc` afterwards to reclaim the disk they used).
    Clear,
}

#[derive(Subcommand, Debug)]
pub(crate) enum TeamCmd {
    /// List every fauto session in this repository: state, task, files touched, overlaps.
    Status,
...
pub(crate) enum WorkCmd {
    /// Create an isolated worktree + branch (`fauto/<name>`) and print where it landed.
    New {
        /// Worktree name: letters, digits, `-`, `_`, `.` (1-64 chars).
        name: String,
    },
    /// List fauto worktrees with their branch, dirty state, unmerged commits, and live sessions.
    List,
    /// Remove an fauto worktree. Refuses while it holds uncommitted or unmerged work.
    Remove {
        /// Worktree name.
        name: String,
...
/// `fauto auth …` — experimental provider OAuth (ChatGPT Codex).
#[derive(Subcommand, Debug)]

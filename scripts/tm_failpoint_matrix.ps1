...
# Requires: target/debug/fauto.exe, git on PATH.
...
$root = if ($PSScriptRoot) { Split-Path -Parent $PSScriptRoot } else { "C:\Users\admin\Desktop\mini_project\fauto" }
if (-not (Test-Path (Join-Path $root "Cargo.toml"))) {
  $root = "C:\Users\admin\Desktop\mini_project\fauto"
}
$fauto = Join-Path $root "target\debug\fauto.exe"
if (-not (Test-Path $fauto)) {
  Write-Host "building fauto..."
  Push-Location $root
  cargo build --bin fauto 2>&1 | Select-Object -Last 5
  Pop-Location
}
if (-not (Test-Path $fauto)) { throw "missing $fauto" }
...
function Invoke-F.Auto {
  ...
  try {
    ...
    $out = & $fauto @CliArgs 2>&1 | Out-String
...
$base = Join-Path $env:TEMP ("fauto-tm-matrix-" + [guid]::NewGuid().ToString("n"))
...
try {
  ...
  $r = Invoke-F.Auto $repo1 $h1 @("time","save","a")
  Assert-True ($r.Exit -eq 0) "save ok"
  $r = Invoke-F.Auto $repo1 $h1 @("time","doctor","--json")
  ...
  $null = Invoke-F.Auto $repo2 $h2 @("time","save","ok")
  ...
  $r = Invoke-F.Auto $repo2 $h2 @("time","list")
  Assert-True ($r.Exit -ne 0) "list rejects corrupt ledger"
  $r = Invoke-F.Auto $repo2 $h2 @("time","save","should-fail")
  ...
  Pop-Location
  $r = Invoke-F.Auto $repo3 $h3 @("time","save","budget")
  ...
  & $git init -q
  Pop-Location
  $r = Invoke-F.Auto $repo4 $h4 @("time","save","nested")
  ...
  Pop-Location
  $r = Invoke-F.Auto $repo5 $h5 @("time","save","filter")
  ...
  $r = Invoke-F.Auto $repo6 $h6 @("time","list")
  ...
  $null = Invoke-F.Auto $repo8 $h8 @("time","save","base")
...
"@
    ...
    $r = Invoke-F.Auto $repo8 $h8 @("time","doctor")
    ...
    $r = Invoke-F.Auto $repo8 $h8 @("time","doctor","--repair")
  ...
  $null = Invoke-F.Auto $repo9 $h9 @("time","save","live")
  ...
  if ($store) {
    ...
    if ($liveRef) {
      ...
      $null = Invoke-F.Auto $repo9 $h9 @("time","gc")
  ...
  $null = Invoke-F.Auto $repo10 $h10 @("time","save","x")
  $r = Invoke-F.Auto $repo10 $h10 @("time","clear")
  Assert-True ($r.Exit -eq 0) "clear ok"
  $r = Invoke-F.Auto $repo10 $h10 @("time","list")
  ...
  if (Test-Path $wt11) {
    ...
    $null = Invoke-F.Auto $repo11 $h11 @("time","save","from-main")
    Set-Content -Path (Join-Path $wt11 "file.txt") -Value "linked-wt" -Encoding ascii -NoNewline
    $null = Invoke-F.Auto $wt11 $h11 @("time","save","from-linked")
    ...
    $r = Invoke-F.Auto $repo11 $h11 @("time","gc")
    Assert-True ($r.Exit -eq 0) "gc from main worktree ok"
    $r = Invoke-F.Auto $wt11 $h11 @("time","list")
    ...
    $null = Invoke-F.Auto $wt11 $h11 @("time","restore","1")
  ...
  $null = Invoke-F.Auto $repo12 $h12 @("time","save","base")
  Set-Content -Path (Join-Path $repo12 "file.txt") -Value "v2" -Encoding ascii -NoNewline
  $null = Invoke-F.Auto $repo12 $h12 @("time","save","second")
  $jobs12 = @()
  1..4 | ForEach-Object {
    $i = $_
    $jobs12 += Start-Job -ScriptBlock {
      ...
      return $LASTEXITCODE
    } -ArgumentList $fauto, $repo12, $h12, $i
  }
  ...
  $r = Invoke-F.Auto $repo12 $h12 @("time","list")
  Assert-True ($r.Exit -eq 0) "ledger readable after concurrent restore/save"
  $r = Invoke-F.Auto $repo12 $h12 @("time","doctor")

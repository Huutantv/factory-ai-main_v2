# F.Auto in a container — for `fauto serve` (the Telegram/Discord daemon) and for running the agent
# with a blast radius you control.
#
# WHY containerize a tool that advertises "no Docker": that promise is about INSTALLING fauto — you
# never need Docker to use it. This image is for the other direction: the agent has a shell and can
# edit files, so a container is the cheapest honest sandbox for `--approval yolo`. And `fauto serve`
# is a long-lived daemon, which is the shape orchestrators are good at.
#
# Build:  docker build -t fauto:local .
# Build without the dense retrieval tier (smaller, faster):
#         docker build --build-arg FEATURES= -t fauto:lean .
...
RUN mkdir -p src/agent \
 ...
 && cargo build --release --bin fauto ${FEATURES:+--features $FEATURES} 2>/dev/null || true

# Now the real source. Touching main.rs defeats any stale-mtime caching of the stub.
COPY src ./src
COPY bench-fixtures ./bench-fixtures
RUN touch src/main.rs \
 && cargo build --release --bin fauto ${FEATURES:+--features $FEATURES} \
 && strip target/release/fauto || true
...
RUN useradd --uid 10001 --create-home --shell /bin/bash fauto

COPY --from=build /src/target/release/fauto /usr/local/bin/fauto
...
ENV AIZEN_HOME=/home/fauto/.aizen
VOLUME ["/home/fauto/.aizen"]

# /work is where the agent operates on your code. Mount your project here.
RUN mkdir -p /work && chown fauto:fauto /work
WORKDIR /work
...
# Reads the heartbeat `fauto serve` writes; see `src/hostbot/health.rs`. `--interval` is longer than
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
  CMD ["fauto", "serve", "--health"]

# tini reaps the agent's grandchildren and forwards SIGTERM to fauto, which now handles it (see
# `hostbot::daemon::shutdown_signal`): kill the process tree, drop the heartbeat, exit clean.
ENTRYPOINT ["/usr/bin/tini", "--", "fauto"]
CMD ["serve"]

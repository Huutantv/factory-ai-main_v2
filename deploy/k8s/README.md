...
Probably not, if the goal is just "keep the bot alive". `fauto serve --install --user --now` writes a
...
It's worth it when you already run a cluster and want fauto managed alongside everything else —
...
| File | What it is |
| --- | --- |
| `namespace.yaml` | The `fauto` namespace. |
...
kubectl -n fauto logs -f statefulset/fauto
```

## Verify

```bash
kubectl -n fauto get pod                        # Running, 1/1
kubectl -n fauto exec statefulset/fauto -- fauto serve --health
# → fauto serve [telegram] healthy — idle 7s ago (pid 1)
```
...
Two things need saying plainly, because they are properties of what fauto *is*:
...
**`AIZEN_ALLOW_PRIVATE_NET` is a bigger switch than it looks.** fauto's SSRF floor
...
```bash
kubectl -n fauto exec statefulset/fauto -- tar cz -C /home/fauto .aizen > fauto-home-backup.tgz
```

## Upgrade

```bash
kubectl -n fauto set image statefulset/fauto fauto=your-registry/fauto:v0.5.1
```

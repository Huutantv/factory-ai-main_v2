# F.Auto — Complete Feature Specification

> **Purpose of this document.** A full, accurate inventory of everything F.Auto does, written as a
...
**F.Auto** (binary `fauto`) is a **single-binary, provider-agnostic agentic coding CLI**. It talks to any **OpenAI-compatible** endpoint (OpenAI, Anthropic via compatible gateway, local servers, etc.), and bundles an unusually deep set of capabilities into one fast, pure-Rust static executable — no Node, no Python, no external `rg`/browser engine required.
...
F.Auto is the same engine exposed through five distinct surfaces. Each needs its own design treatment.

| Surface | Entry | Character | Design implication |
|---|---|---|---|
| **Interactive TUI / REPL** | `fauto agent` (or bare `fauto` after setup) | Sticky-footer chat with live slash palette, HUD, streaming markdown | The flagship screen — most UI work lives here (§4) |
| **Landing menu** | bare `fauto` (first run / not configured) | Branded splash + arrow-key setup wizard | First impression; onboarding flow (§4.2) |
| **One-shot commands** | `fauto chat`, `fauto memory …`, `fauto crawl`, etc. | Plain stdin/stdout, pipe-safe, no chrome | Docs-driven; clean text output, scriptable |
| **Always-on daemons (bots)** | `fauto serve` (Telegram), `fauto discord serve` | Headless; chat from your phone/Discord, approvals via inline buttons | Mobile/remote UX; setup wizards |
| **Scheduled jobs** | `fauto cron add …` (OS scheduler) | Unattended; logs to file | Config + log-viewing UX |
...
**The agent loop — `fauto agent <task>`**
...
- **Verify gate (post-edit typecheck):** after a successful destructive edit and before declaring "done," F.Auto auto-runs the project's check (`cargo check`; for Node, `tsc --noEmit` / typecheck script; silent no-op for unknown projects, 90s timeout). On failure it injects the compiler errors and grants one fix turn. This is the "done but broken" catcher.
...
**Multi-agent workflow — `fauto workflow <spec.json>`**
...
### E. Personas & self-memory — `fauto persona …`
...
### H. Apps & MCP — `fauto apps …` / `fauto mcp …`
...
**`fauto apps` subcommands:** `list` (featured + custom, ✓/○ connection badges), `search <keywords>`, `add <name>` (interactive: pick a server with a ★-recommended local-first default, runtime-on-PATH check, explicit third-party/OAuth confirm gates, masked-secret review before writing), `info <key>` (config + **live probe** of the tools it exposes), `login <key>` (OAuth), `remove <key>`.

**`fauto mcp` subcommands:** `list` (connected servers + tools + lifecycle generation/health/schema pin), `login <name>`, `trust` / `untrust` (the supply-chain gate for project-local servers).
...
**Telegram daemon — `fauto serve`** — the long-lived bot: long-polls Telegram, runs the agent per message, keeps **persistent per-chat history** (follow-ups like "now fix it" keep context, capped ~40 msgs). In-chat commands: `/help`·`/start`, `/new`·`/reset`, `/resume`, and `/agent <task>` (autonomous mode). **Destructive-op approvals route to inline ✓/✗ buttons on your phone.** Replies use Telegram-native HTML (headings/emphasis, safe links, inline/fenced code, lists/quotes); tables become compact stacked records, and balanced block-aware chunks stay ≤3500 UTF-16 units. One temporary working message is deleted after delivery; rich-format rejection retries once as equivalent plain text.
- Setup — `fauto telegram setup` (paste BotFather token → message the bot to capture your chat id), `test`, `show`. Allowed-chat allowlist (empty = deny everyone).

**Discord bot — `fauto discord …`** — two-way gateway bot (`setup`/`test`/`serve`/`show`/`disable`); needs the privileged MESSAGE_CONTENT intent; channel allowlist; same in-chat commands; replies chunked to ≤1900 chars. (Inline-button approvals not yet wired — use `/agent` for autonomous edits.)
...
**Models — `fauto models`** — lists provider models (with context windows when advertised). Feeds the `/model` picker.
...
the alternate screen fails) fauto runs the plain line-REPL instead. The input/footer is pinned at the bottom with a HUD;
PageUp/PageDown scroll 4 lines and End jumps to the live tail when the draft is empty or the caret is already at end-of-draft (otherwise it moves the caret there first) — and when an informational
...
magenta left gutter `▌` marks assistant lines (vs `❯` user echoes and `⚙ name  target  digest` tool
lines whose right-aligned digest is tinted by outcome — green ok / red error — plus boxed plan / diff
...
- **Keys:** Enter submit · Esc cancel running turn / clear draft · Ctrl-C cancel · Ctrl-D quit-if-empty · Tab complete slash command · ↑/↓ or Ctrl-P/Ctrl-N history recall · Home/End/←/→ edit (End is context-aware: empty draft or caret at end → live tail).
- **Working indicator:** a magenta star-spinner pill with a cycling verb, live elapsed clock and output-token counter — `✻ Đang nghiền ngẫm… · 23s · ↑1.2K tok · Esc`.
...
scratch → how to do things* — is the conceptual heart of F.Auto.
...
| Command | Summary |
|---|---|
| `fauto` | Landing menu / interactive REPL (after setup). |
| `fauto chat [--prompt]` | One-shot streaming chat. |
| `fauto agent <task> [--yes --max-iters -m]` | Run the agentic loop. |
| `fauto workflow <spec.json> [--trace --yes -m]` | Multi-agent fan-out + synthesis. |
| `fauto memory <add\|list\|show\|search\|frozen\|learn\|style\|profile\|ask\|review\|as-of\|supersede\|archive\|restore\|compact>` | The memory brain. |
| `fauto skill <list\|show\|add\|delete\|fetch\|search\|install>` | Reusable procedures. |
| `fauto persona <list\|show\|new\|use\|clear\|self\|remember\|block>` | Characters + self-memory. |
| `fauto soul <show\|set\|clear\|path>` | Durable operating identity. |
| `fauto config [set\|show\|path]` · `fauto models` | Endpoint setup / list models. |
| `fauto apps <list\|search\|add\|info\|login\|remove>` · `fauto mcp <list\|login\|trust\|untrust>` | Apps & MCP. |
| `fauto serve` · `fauto telegram <setup\|test\|show>` · `fauto discord <setup\|test\|serve\|show\|disable>` | Bots. |
| `fauto crawl <url…>` | Website crawler. |
| `fauto time <save\|list\|restore\|undo\|redo\|prune\|doctor\|gc\|clear>` | Crash-recoverable Time Machine with private external store. |
| `fauto cron <add\|list\|remove>` | OS-scheduled jobs. |
| `fauto bench <memory\|profile\|dialectic>` | Quality benchmarks. |
...
1. *Getting started* — install (one binary), `fauto config`, first chat.

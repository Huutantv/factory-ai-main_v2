...
## Interactive REPL (just run `fauto`)
...
The chat box is a small line editor: type / Backspace / Del at the cursor, **←/→** move, **Home** jumps
to line start, **↑/↓** (or **Ctrl-P/Ctrl-N**, readline-style) recall past prompts, Enter sends. **End** is
context-aware: with an empty draft — or with the caret already at the end of a non-empty one — it
jumps the view back to the live transcript tail; otherwise it moves the caret to end-of-draft. While
you scroll the transcript, `↑N/↓N` on the footer rule counts the hidden rows. A braille spinner
(`⠹ thinking`) shows while the model is responding, clearing the moment the first token streams.
...
move around in it. Scrolling the transcript — the **mouse wheel**, **PageUp** and **PageDown** — shares
one step of **4 rows** (a `bottom` chip appears once you are back at the live tail).

**Attach an image** (vision) — two ways, because Ctrl-V can't be used (Windows Terminal intercepts
it for its own paste, so the keystroke never reaches `fauto`):
...
- **Drag an image file onto the window** — the terminal pastes the file path; press Enter and `fauto`
...
128K). Override it explicitly with `fauto config set --context-window <tokens>`.
...
| `/timemachine` · `/checkpoint [note]` · `/diff` | `/timemachine` lists every crash-recoverable, worktree-scoped Git checkpoint and jumps back to the code **and** chat of the one you pick (one gesture, reversible); `/checkpoint` saves one now; `/diff` (or `fauto time diff`) shows what changed between two checkpoints, or `working` for the live tree. CLI: `fauto time doctor` inspects without touching the tree and reports loose objects once they pile up; `fauto time gc` compacts this repo's store (packs loose objects — a save does it automatically past 2,048); `fauto time gc --all` sweeps orphaned stores left by deleted/moved repos (dry-run by default, `--apply` moves them to a trash dir, which you then delete to reclaim the space) |
...
| `/cost` | session token usage + a $ estimate (real provider usage when reported; set rates via `fauto config set --price-in/--price-out`) |
| `/layout [name|list]` | pick the TUI layout density: `default` / `balanced` / `polished` / `dense` / `spacious` / `auto` (bare `/layout` opens the arrow-key picker; persisted in cli-config, default `balanced`) |
...
`fauto config set --context-window <tokens> --compact-threshold <0–95>` (`0` = off).

The REPL needs a real terminal; piped/CI stdin prints a hint and exits (`AIZEN_MENU=1` forces it).

**Icons** — the TUI uses a curated glyph set. Pick the style in `/config` or `fauto config set --icons
...
`fauto config set --response-visuals <auto|always|off>`. Wide terminals get boxed tables; narrow
...
## Telegram — control `fauto` from your phone

`fauto serve` runs a long-lived daemon that listens on a Telegram bot (long-poll, no public URL): send
...
HTML is ever rejected, F.Auto retries the same content as plain text. Pure-Rust (no teloxide), single binary.

```bash
fauto telegram setup     # paste the @BotFather token, message the bot to capture your chat id
fauto telegram test      # send a test message
fauto serve              # start listening (Ctrl-C to stop)
```
...
`fauto serve` is a foreground process — to keep the bot alive across logout, crashes, and reboots, run
it as a systemd service:

```bash
fauto telegram setup                    # once: token + your chat id
fauto serve --install --user --now      # write + enable the user unit, start now
```
...
system unit (prints the `sudo` steps unless you're already root). `fauto serve --uninstall --user`
...
Two volumes matter. `fauto-home` (`/home/fauto/.aizen`) holds everything that must outlive the
...
Health is `fauto serve --health`, wired as the image's `HEALTHCHECK`. It reads a heartbeat the daemon
...
kubectl -n fauto create secret generic fauto-secrets \
...
kubectl -n fauto logs -f sts/fauto    # pairing code
```
...
**ChatGPT Codex (experimental)** uses ChatGPT/Codex *consumer* OAuth (not the OpenAI Platform API key). Pick **ChatGPT Codex (experimental)** in `fauto config` → Providers & connection: it skips the API-key prompt and offers the browser sign-in in place of it, so no separate command is needed. The manual equivalents still work — `fauto auth login codex`, or `fauto config set --base-url https://chatgpt.com/backend-api/codex --api-key codex-oauth --model gpt-5.4-mini`. Model ids come from a shipped catalog, since the Codex backend has no `GET /models`. Tokens live in `~/.aizen/provider-tokens/codex.json` and `fauto config show` reports whether you are still signed in. Kill-switch: `AIZEN_DISABLE_CODEX=1`. **Risk:** private backend APIs may break or conflict with vendor terms; prefer Platform API keys / OpenRouter for supported production use. Logout: `fauto auth logout codex`.
...
### `fauto config` — provider-first setup (recommended)
...
```bash
fauto config            # Providers & connection → Add provider
```
After this, `fauto chat`/`agent`/`workflow` work with **zero env vars**. Non-interactive equivalents:
```bash
fauto config set --base-url https://api.openai.com/v1 --api-key sk-... --model gpt-4o-mini
fauto config show       # API key masked
fauto config path
```
...
```bash
fauto config provider add primary --base-url https://api.openai.com/v1 --api-key sk-... --model gpt-4o-mini --use
fauto config provider add backup --base-url https://backup.example/v1 --api-key bk-... --model model-x
fauto config provider list
fauto config provider use backup
fauto config provider edit backup --base-url https://backup-2.example/v1 --api-key bk-2... --model model-y
fauto config provider rename backup secondary
```
...
saved selection; F.Auto prints a note when those environment variables mask a switch.
...
`big-pickle`; `fauto models` and the model pickers tag those rows with `· free` so a free-tier id
...
```bash
fauto agents set-provider code-reviewer backup              # provider default model
fauto agents set-provider code-reviewer backup model-y      # model override
fauto agents set-provider code-reviewer --clear             # inherit sub-agent default
```
...
Gateways differ in how they shape their streaming frames, and F.Auto absorbs the differences quietly:
...
### `fauto models` — list the provider's models
```bash
fauto models                       # GET {base}/models, marks your default
fauto config set --model <id>      # pick one as the default
```
...
recall unchanged. Verify with `fauto bench memory --split all`.
...
prompt prefix holds the facts you actually use. This is provable, not marketing: `fauto bench memory
--evolution` runs a 6-session reuse simulation and **fails** unless recall@5 climbs ≥5%/session
until it plateaus.

## Commands

### `fauto chat` — one-shot streaming chat
```bash
fauto chat -p "explain this error: ..."
echo "summarize this" | fauto chat        # prompt from stdin
```

### `fauto agent` — the tool-using loop
The model reads/edits files, runs shell, and uses memory to finish a task end-to-end.
```bash
fauto agent "add a --version flag and update the help text"
fauto agent --yes "fix the failing test in src/parse.rs"   # pre-approve file/shell ops
fauto agent --max-iters 40 "..."                            # raise the step cap
fauto agent --save-session "..."                            # keep the transcript for /sessions
fauto agent --effort high "..."                             # this run only; the config is untouched
fauto agent --image shot.png "why is this button misaligned?"  # vision: repeat --image for more
```
...
- **Approval** — destructive tools (`file_edit`, `shell_run`) prompt before running. In the sticky
  ...
  under `fauto serve` the prompt is routed to your phone. The hard `cmd_guard` floor blocks catastrophic
...
- **Web research** — `web_search` (needs a free Tavily key — set `TAVILY_API_KEY`) finds pages; `web_fetch` GETs a URL and
  ...
  seed URL (see `fauto crawl` below). Read-only; available to every role.

### `fauto workflow <spec.json>` — fan-out + synthesis
...
```bash
fauto workflow examples/review.workflow.json
```
Spec shape:
```jsonc
{
...
### `fauto crawl <url>` — katana-style web crawler
...
```bash
fauto crawl https://example.com                       # depth 2, same host, ≤200 URLs
fauto crawl https://example.com --depth 1 --max-pages 50 --show-source
fauto crawl https://example.com --scope subs          # also follow *.example.com subdomains
fauto crawl https://example.com --json                # [{url, depth, via}]
```
...
### `fauto sandbox` — the OS sandbox around model-run commands

```bash
fauto sandbox status          # live capability matrix: enforced / partial / advisory / unavailable
fauto sandbox doctor [--json] # probe backend, audit-log check, real env-scrub self-test, tmp sweep
fauto sandbox explain         # how the next shell_run would be sandboxed, step by step
fauto sandbox run -- <cmd>    # run one command under the sandbox by hand
fauto --sandbox strict …      # per-invocation mode override (also AIZEN_SANDBOX, /sandbox, config)
```
...
(`fauto config set --persona-evolve false`). Honest framing: this is a *consistent, accumulating,
...
```bash
fauto persona new Aria -r "a sharp mentor" -v "concise, warm" -b "You value clarity."  # body via -b or stdin
fauto persona use Aria / clear                         # set / clear the active persona
fauto persona list / show <name>                        # list (● active, with self-memory counts) / show a card
fauto persona self [name]                               # view accumulated insights + recent episodes
fauto persona remember "what I just lived through"      # record a free episode (auto-scored importance)
fauto persona block                                     # print the <persona> + <self> blocks the model sees
```

### `fauto soul` — the agent's operating identity
...
```bash
fauto soul set -b "Always run tests before saying done. Never push without asking."  # body via -b or stdin
fauto soul show        # print the <agent_identity> the model actually sees
fauto soul path        # ~/.aizen/SOUL.md — edit directly in any editor
fauto soul clear
```

### `fauto skill` — reusable procedures (skills)
...
```bash
fauto skill add deploy-vps -d "ship over SSH" -w "asked to deploy"   # body from --body or stdin
fauto skill fetch https://example.com/deploy-vps.md                  # pull a shared skill from a URL
fauto skill list / show <name> / delete <name>
fauto skill where                                                    # the three folders + counts
```
...
self-learning, but for how-to. Toggle in `/config` (default on) or `fauto config set --auto-skill-learn false`.
...
`fauto` can use tools from any [Model Context Protocol](https://modelcontextprotocol.io) server. Declare
...
At startup `fauto` connects each enabled server, lists its tools, and exposes each one to the agent as
...
Node/Python MCP SDK, no extra runtime; the single static binary is preserved. `fauto mcp list` or
...
F.Auto defers the new schema until the next fresh user message instead of mutating the tool registry
...
are OAuth-only. `fauto` speaks the full OAuth 2.1 (PKCE) flow: an entry with `"auth": "oauth"` triggers a
browser sign-in (`fauto apps login <key>`, or automatically when you `fauto apps add` one), caches the token
...
**Connect apps without editing JSON** — `fauto apps` is a curated catalog over the official MCP registry:
`fauto apps list` (featured + connected), `fauto apps search <kw>`, `fauto apps add <key|name>` (picks a server,
prompts only for real secrets, signs you in if it's OAuth), `fauto apps info <key>` (config with secrets
masked + a live tool probe), `fauto apps login <key>`, `fauto apps remove <key>`. Also the `/apps` TUI hub.
...
### `fauto memory` — the self-learning brain
```bash
fauto memory add "prefer-pnpm" -t feedback -b "I prefer pnpm over npm"
fauto memory list [current|global|project|<zone>]   # narrow a long listing to one workspace view
fauto memory search "package manager" [--dimension tooling]
fauto memory profile [--json]      # derived preferences rollup (verbosity/tooling/stack/…)
fauto memory ask "which package manager should I use?"   # abstains rather than guessing
fauto memory learn "<a user turn>"  # free extraction → threat-scan → route → store
fauto memory frozen                 # the always-on prompt-prefix core
fauto memory style | review | as-of <date> | supersede <old> <new> | archive | restore <id> | compact
fauto memory where                  # the folders + counts, for editing or clearing out many at once
```
...
### `fauto bench` — anti-oracle benches
```bash
fauto bench memory [--split gate|tune|all] [--hybrid]   # retrieval recall vs a baseline
fauto bench memory --evolution                          # multi-session reuse gate (≥5%/session lift)
fauto bench profile                                     # golden set for the profile rollup
fauto bench dialectic                                   # golden set incl. abstain-when-unknown
```
...
every model/repo-influenced child runs with F.Auto's secrets scrubbed from its environment, a
...
platforms without a kernel backend. `fauto sandbox status|doctor|explain` show what *your*
...
tools (`web_fetch`/`web_crawl`/`fauto crawl`) carry an **SSRF floor**: a URL that resolves to a
...
checkout's `.git/hooks` never executes because F.Auto made a checkpoint. Tool results and file
contents are treated as data, never as instructions.

## Remote control & notifications
`fauto serve` / `fauto discord serve` run long-lived daemons that drive the agent from Telegram or a
Discord bot (pure-Rust gateways, no SDKs); destructive ops ask for approval from your phone. `fauto

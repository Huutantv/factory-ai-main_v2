# Hướng dẫn sử dụng F.Auto

F.Auto là trợ lý lập trình chạy trong terminal. Bạn gõ yêu cầu bằng tiếng Việt (hoặc tiếng Anh), F.Auto tự đọc file, viết code, chạy lệnh, kiểm thử — trong một vòng làm việc liền mạch. Tài liệu này đi qua **tất cả** tính năng, từ lúc mở lên tới các lệnh nâng cao.

> Quy ước: `❯` là dấu nhắc bạn gõ vào. `/lệnh` là lệnh gõ trong phiên chat. `fauto <lệnh>` là lệnh gõ ở terminal (ngoài phiên chat).
...
F.Auto mở ra giao diện chat. Nếu chưa cấu hình endpoint/key, chạy trình cài đặt:
...
F.Auto sẽ tự quyết định đọc file nào, sửa gì, chạy test ra sao. Bạn không cần chỉ định từng bước.
...
Ngoài ra còn hai tiền tố đặc biệt hoạt động **khi F.Auto đang chạy một lượt** (xem mục 4):
...
Nội dung file được chèn thẳng vào tin nhắn trước khi gửi cho F.Auto. Bạn cũng có thể chèn output một lệnh chỉ-đọc bằng cú pháp `` !`lệnh` `` ngay trong câu:
...
| `Ctrl-P` / `Ctrl-N` | Duyệt lịch sử dòng đã gõ (giống `↑`/`↓`, kiểu readline) |
| `PageUp` / `PageDown` | Cuộn transcript 4 dòng (chuột lăn cũng 4 dòng) |
| `End` | Đang xem lịch sử (draft rỗng hoặc caret đã ở cuối) → về cuối live; ngược lại → caret về cuối dòng |
...
Đây là hai tính năng để bạn tương tác **trong lúc F.Auto đang làm việc**, và chúng khác nhau về bản chất:

### `>` Điều hướng (steering) — đổi hướng công việc

Khi F.Auto đang chạy mà bạn muốn bổ sung/đổi ý:
...
Ý này được **gấp VÀO lượt đang chạy** — F.Auto điều chỉnh ngay, không cần chờ nó xong rồi bắt đầu lại. (Cũng làm được bằng `Alt+Enter` / `Ctrl+Enter`.)

### `?` Hỏi ngoài lề (aside) — hỏi mà không ảnh hưởng công việc

Khi F.Auto đang refactor mà bạn chợt có câu hỏi **không liên quan** tới task:
...
Khi F.Auto đang rảnh (không chạy lượt nào), gõ `? ...` chỉ đơn giản là một câu hỏi thường.
...
Ở terminal:
```
fauto config set --price-in <giá> --price-out <giá>   # đặt đơn giá để /cost tính tiền
fauto models                                          # liệt kê model provider quảng bá
```
...
F.Auto nhớ các sự thật lâu dài về bạn và dự án, và tự dùng lại ở các phiên sau.
...
Nhanh hơn: dùng tiền tố `#` (mục 2). Ở terminal có `fauto memory ...` với các subcommand tương ứng (`edit`, `forget`, `purge`…).

---

## 7. Persona & Soul (tính cách và bản sắc)

- **`/persona`** — chọn "nhân vật" mà F.Auto nhập vai (liệt kê · chọn · tạo mới · xóa). Mỗi persona có bộ nhớ tự tiến hóa riêng.
- **`fauto soul`** (ở terminal) — bản sắc vận hành lâu bền tại `~/.aizen/SOUL.md`: các giá trị/nguyên tắc áp dụng cho **mọi** persona và **mọi** dự án.

---

## 8. Kỹ năng & lệnh tùy biến

- **`/skills`** — các quy trình từng-bước đã lưu để F.Auto nạp khi cần (liệt kê · xem · tạo · xóa).
...
F.Auto chụp ảnh code theo thời gian (dựa trên git), cho phép quay lại bất kỳ điểm nào — cả code lẫn nội dung chat.
...
Ở terminal: `fauto time save · list · restore · undo · redo`.
...
```
/goal <mô tả>       # chạy tới khi đạt mục tiêu, F.Auto tự tuyên bố hoàn thành + tự kiểm chứng
/goal off           # dừng
```

Không giới hạn số vòng lặp, tự thử lại khi lỗi API. F.Auto chỉ dừng khi tự tuyên bố xong (`goal_complete`) VÀ bước kiểm chứng đạt. `Esc` để hủy.
...
| `/team` | Các cửa sổ F.Auto khác đang làm trong repo này — xem file, diff, commit công việc của chúng | |
| `/work` | Worktree git tách biệt, mỗi phiên một cái (list · new · remove) | |

`/team` giúp nhiều phiên F.Auto làm chung một repo mà quy được công đúng người; `/work` cô lập mỗi task vào một worktree riêng để không giẫm chân nhau.

Ở terminal: `fauto agents ...`, `fauto team ...`, `fauto work ...`, `fauto workflow ...`.
...
Ở terminal:
```
fauto apps list | search <từ khóa> | add <tên> | info <tên> | login <tên> | remove <tên>
fauto mcp ...
```
...
F.Auto có thể chạy như một daemon: lắng nghe Telegram/Discord, chạy agent trên tin nhắn đến, và **định tuyến các yêu cầu duyệt lệnh nguy hiểm về điện thoại bạn**.
...
Ở terminal:
```
fauto serve                       # chạy daemon
fauto serve --token <token>       # dán token & chạy luôn (owner ghép cặp trong chat)
fauto serve --install [--now]     # cài thành dịch vụ systemd (Linux) — tự khởi động lại + tự chạy khi boot
fauto serve --health              # kiểm tra daemon còn sống (dùng cho probe container/systemd)
fauto telegram ...
fauto discord setup | test | serve | show | disable
```
...
Ở terminal có `fauto crawl <url>` để crawl một website (BFS qua HTTP, rút link từ HTML + endpoint từ JS).
...
```
fauto where
fauto zone report            # báo cáo các zone (slug key cho memory/skills/index)
fauto zone migrate           # gộp các "zone song sinh" cũ (dry-run mặc định, --apply để làm thật)
```
...
```
fauto cron add ...           # lập lịch task qua bộ lập lịch của HĐH (không cần daemon)
fauto cron list
fauto cron remove ...
```
...
```
/update          # (hoặc `fauto update`) hiện phiên bản đang cài cạnh mọi phiên đã phát hành, chọn cái để cài (mới hơn HOẶC cũ hơn)
```
...
| `/quit` | Thoát F.Auto (`/exit`, `/q`) |
| `/art` (hoặc `fauto art`) | Vẽ khung cảnh nghệ thuật braille dưới ánh trăng |
...
1. `/init` để F.Auto hiểu codebase (lần đầu).

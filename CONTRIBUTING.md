# Contributing to F.Auto

Thanks for wanting to help. F.Auto is **open source under the
...
- **Search existing [issues](https://github.com/Huutantv/factory-ai/issues)** before opening a new one —
...
F.Auto asks every contributor to agree to the **[Contributor License Agreement](CLA.md)** once,
...
- **But** you also grant the maintainer the right to license your contribution under *other* terms,
  **including proprietary commercial ones**. Concretely: a paid commercial edition of F.Auto may
...
F.Auto is a **pure-Rust single static binary** — no C toolchain, no external runtime deps.

```bash
# build
cargo build --release --bin fauto

# run the full test suite (should be green before you push)
cargo test --bin fauto

# the semantic-retrieval tier is behind a feature flag; build it if you touch that code
cargo build --release --features dense --bin fauto
```
...
4. Run `cargo test --bin fauto` and make sure it's **green**.
...
7. Push and open a PR against `Huutantv/factory-ai:dev`. Fill in the PR template.

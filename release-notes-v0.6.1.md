## F.Auto v0.6.1 — the quiet wrong answers
...
- **Four checkpoint tools collapse into two.** `checkpoint` takes `action=save|rewind|restore` (approval-gated); read-only `checkpoint_view` takes `diff|list` and stays out of the approval path. `fauto time …` is unchanged.
...
- Human-facing commands (`fauto time save|list|restore|undo|redo`, `/diff`) are unchanged.
